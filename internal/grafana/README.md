...
- The `discord` notification template group (`tldraw.discord.title` / `tldraw.discord.text`), which compresses alert notifications to severity + summary + View/Silence links and explains `DatasourceNoData` as a likely metrics-source outage. The Discord contact point's Title and Message Content fields reference these templates; that reference is set by hand (contact points stay hand-managed), so the templates only take effect while the contact point points at them.
