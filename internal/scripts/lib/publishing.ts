...
export async function publish(distTag?: string) {
	...
	// CI must grant `permissions: id-token: write` so npm CLI (>= 11.5.1, bundled
	// with our Node version) can exchange the GitHub-issued OIDC token for a
	// short-lived publish token automatically. Provenance attestations are
	// generated by trusted publishing automatically, so we don't pass
	// `--provenance` explicitly. No NPM_TOKEN is required.
	...
	for (const packageDetails of publishOrder) {
		...
		await retry(
			async () => {
				...
				try {
					await exec(`npm`, ['publish', '--tag', String(tag), '--access', 'public'], {
