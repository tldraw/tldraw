...
const oxfmtConfig = json5.parse(readFileSync(join(REPO_ROOT, '.oxfmtrc.json'), 'utf8'))
export async function writeCodeFile(
	...
	const result = await format(`file.${ext}`, source, oxfmtConfig)
	await writeStringFile(filePath, result.code)
}
...
export async function writeJsonFile(filePath: string, contents: unknown) {
	await writeStringFile(filePath, JSON.stringify(contents, null, '\t') + '\n')
}
