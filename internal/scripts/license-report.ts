...
async function getWorkspaces(): Promise<string[]> {
	...
	return packages
		...
		.filter((location) => location.startsWith('packages/'))
		.filter((location) => {
...
async function getReportRows(location: string, only: string): Promise<ReportRow[]> {
	// `pnpm exec` rather than `pnpm license-report`, which would resolve to the root script that
	// runs this file.
...
{/* This page is generated by \`pnpm license-report --prod\`. Do not edit it by hand. */}
...
async function main() {
	...
	for (const location of await getWorkspaces()) {
	...
	if (prodOnly) {
		...
		// next `pnpm format` and then reverted by the next regeneration.
		await exec('pnpm', ['exec', 'oxfmt', '--write', DOCS_PAGE_PATH], {
			processStdoutLine: () => {},
			processStderrLine: () => {},
		})
