...
### CSS variable prefix change ([#6712](https://github.com/tldraw/tldraw/pull/6712))

All CSS variables now start with `--tl-`. Update custom CSS that references tldraw variables:

- `--color-background` → `--tl-color-background`
- `--space-4` → `--tl-space-4`

### Geometry API update ([#6668](https://github.com/tldraw/tldraw/pull/6668))

Replace `Geometry2D.isLabel` with the `excludeFromShapeBounds` flag.

### Open URL event change ([#6641](https://github.com/tldraw/tldraw/pull/6641))

The `open-url` event now uses `destinationUrl` instead of `url`.

### Arrow text migration ([#6594](https://github.com/tldraw/tldraw/pull/6594))

Arrow shapes now use `richText` instead of `text` property. Data migrates automatically, but direct access to `arrow.props.text` must update to `arrow.props.richText`.

### AI module removal

The `@tldraw/ai` module has been removed. Use the [agent starter kit](https://tldraw.dev/starter-kits/agent) instead.

### Event propagation changes ([#6592](https://github.com/tldraw/tldraw/pull/6592))

Event propagation behavior has changed. Use `editor.markEventAsHandled()` for custom event handling.

### Style panel redesign ([#6573](https://github.com/tldraw/tldraw/pull/6573))

The style panel has been redesigned. Legacy picker components have been removed.

### Toolbar class names ([#6551](https://github.com/tldraw/tldraw/pull/6551))

Toolbar CSS class names have been updated for consistency.
