...
This month's release includes an upgrade to React 19, significant performance improvements, SQLite storage support for sync, and new editor APIs for reactive inputs and debounced zoom.

### React 19 ([#7317](https://github.com/tldraw/tldraw/pull/7317))

We've upgraded tldraw from React 18 to React 19. This includes updated peer dependencies and some type signature changes for React 19 compatibility:

- Ref types now include `| null` union (e.g., `RefObject<HTMLElement | null>`)
- Ref callback return types must return `void`
- `useRef` calls with optional types require explicit `undefined`

If you're still on React 18, tldraw should continue to work, but we recommend upgrading to React 19 for the best experience.
...
- Rename `startEditingShapeWithLabel` to `startEditingShapeWithRichText` and update signature to accept `options` parameter with `{ info, selectAll }`. ([#7361](https://github.com/tldraw/tldraw/pull/7361), [#7397](https://github.com/tldraw/tldraw/pull/7397))

- Refactor `Editor.getCanUndo()`/`Editor.getCanRedo()` to `Editor.canUndo()`/`Editor.canRedo()`. ([#7361](https://github.com/tldraw/tldraw/pull/7361))

- Add augmentation-based shape and binding types for stricter TypeScript typing of custom shapes. ([#7091](https://github.com/tldraw/tldraw/pull/7091)) (contributed by [@Andarist](https://github.com/Andarist))
