...
### Image and video storage improvements

When using local-only storage, images and videos are now stored in a separate IndexedDB table instead of as base64-encoded blobs in the room's JSON. This significantly improves performance and reduces the size of room data. ([#3836](https://github.com/tldraw/tldraw/pull/3836))

### Fonts and icons CDN

We've improved our CDN for assets (fonts and icons). We were using unpkg before, but now we're using Cloudflare directly for more fine-grained control and better performance. ([#3923](https://github.com/tldraw/tldraw/pull/3923), [#3927](https://github.com/tldraw/tldraw/pull/3927))

### Image level-of-detail (internal)

We've been working on "level-of-detail" for images. For higher-resolution images (over a couple megabytes), the image gets transformed to an appropriate size depending on your viewport, zoom level, and network speed. This helps with bandwidth and browser memory usage when you have multiple high-res images on the board. ([#3827](https://github.com/tldraw/tldraw/pull/3827))

### Flatten

You can now select multiple shapes and flatten them into a single image. For those moments when you need to press things together. ([#3933](https://github.com/tldraw/tldraw/pull/3933))

### Security improvements

We locked down referrer network requests to make sure we weren't leaking tldraw rooms to external media/iframe requests. ([#3884](https://github.com/tldraw/tldraw/pull/3884), [#3881](https://github.com/tldraw/tldraw/pull/3881))
