...
potential: 9
accuracy: 9
notes: "Excellent improvements. Much lighter, more humble tone throughout. 'What we ended up building' and 'turns out to be' soften claims nicely. No harsh declaratives. All technical details verified accurate."
---
...
What we ended up building to make this work is a project management system. We call the agents "fairies" because they're childlike and mischievous—appropriate for the current era where models aren't actually good at this yet. This is what we learned about coordinating multiple agents on a spatial canvas.

## The underwater problem

Agents spend far more time writing than reading. While an agent streams its response and executes actions, it can't perceive the canvas. It surfaces for context, acts, then goes back under.
...
This turns out to be fundamentally different from human collaboration. You can't give agents real-time feedback while they work. You can only interrupt them and force them to resurface with fresh context.

## Roles, modes, and projects
...
What worked better was when we watched agents succeed in a different scenario: one agent would make a plan and ask specific agents for help on specific items. So we built a project management system that separates concerns.
...
**Roles**: With multiple agents, we have an orchestrator and workers. The orchestrator creates projects, breaks them into tasks, and assigns spatial bounds. Workers execute tasks within their assigned regions.
...
The orchestrator can't edit the canvas—it can only see, move around, and manage tasks:
...
Separating concerns this way means orchestrators can focus on the big picture without getting lost in implementation details. Workers focus on their assigned region without getting distracted by what other agents are doing.

## Spatial coordination

The canvas adds a constraint: agents need to know where to put things. When an orchestrator creates tasks, it assigns spatial bounds—a rectangular region where the agent should work. This helps with more than just preventing overlap. The composition needs to be coherent.
...
Interrupts address the underwater problem: we can't give agents real-time feedback, but we can make them surface for air when the world has changed.
...
LLM performance degrades as context length increases, even on simple tasks. Adding irrelevant context forces the agent to figure out what matters before it can act.

We filter context based on scope using three memory levels:
...
Memory levels keep context focused and prevent context rot. The orchestrator never sees the minutiae of how workers positioned shapes. Workers don't carry baggage from previous tasks. Each agent gets exactly the context it needs for its current scope.
