...
ShapeUtils can override capability methods to declare what interactions the shape supports. These methods return booleans indicating whether the shape can be edited, resized, rotated, cropped, scrolled, or bound to other shapes. The `canReceiveNewChildrenOfType` method controls whether the shape can contain other shapes as children. Custom hit testing logic can be provided through `hitTestPoint`.
...
The `onResize` hook requires careful implementation. When a shape resizes, the hook receives resize information including the scale factors and the handle being dragged, and must return a new shape with updated dimensions:

```typescript
onResize(shape: MyShape, info: TLResizeInfo<MyShape>): MyShape {
	return {
		...shape,
		props: {
			...shape.props,
...
Shape opacity multiplies with parent opacity. A shape at 50% opacity inside a frame at 50% opacity renders at 25% opacity. The editor tracks this computed opacity for rendering and provides it through `getShapeOpacity`.
...
Frames are container shapes that clip their children and provide a visual boundary. Shapes inside a frame position relative to the frame's origin. Moving the frame moves all its children. The frame clips content at its boundaries during rendering.
...
The editor tracks a **focused group** that defines the current editing scope. When you double-click a group, it becomes focused, and you can select and edit shapes inside it. Get the current focused group with `getFocusedGroup`. Focus a specific group with `focusGroup`. Exit the focused group with `popFocusedGroupId`.
...
Tracks which shapes are outside the viewport. Shapes with `hideInCulledBounds` set to `true` are candidates for culling. Check if a shape is currently culled with `isShapeCulled`.
...
- packages/editor/src/lib/editor/shapes/ShapeUtil.ts - Base ShapeUtil class
...
- packages/editor/src/lib/editor/Editor.ts - Shape methods and derivations
...
- [Selection system](./selection-system.md) - How shapes get selected
- [Shape indexing](./shape-indexing.md) - Z-order with fractional indices
- [Shape transforms](./shape-transforms.md) - Grouping and transformation operations
- [Bindings](./bindings.md) - Relationships between shapes
- [Coordinate systems](./coordinate-systems.md) - Space transformations
- [Custom shapes guide](../guides/custom-shapes.md) - Tutorial for creating custom shapes
