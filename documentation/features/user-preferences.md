...
The system validates preferences using `userTypeValidator` from `@tldraw/validate`. When loading preferences from localStorage or receiving updates, validation ensures all fields conform to expected types. Invalid data triggers fallback to fresh preferences rather than causing errors or unexpected behavior.
