...
/** @public */
export const DefaultColorThemes: Record<string, TLDefaultColorTheme> = {
	light: {
		id: 'light',
		text: '#000000',
		background: '#f9fafb',
...
export function getDefaultColorTheme(opts: {
	colorScheme: keyof typeof DefaultColorThemes
}): TLDefaultColorTheme {
	return DefaultColorThemes[opts.colorScheme]
}
...
export function extendDefaultColorTheme(
	newColors: Record<
		string,
		Partial<TLDefaultColorTheme> & { colors: Record<string, Partial<TLDefaultColorThemeColor>> }
	>
) {
	for (const [themeName, theme] of Object.entries(newColors)) {
		// deep merge into existing theme

		const prev = DefaultColorThemes[themeName]

		DefaultColorThemes[themeName] = {
			...prev,
			...theme,
			colors: {
				...prev.colors,
		...
		for (const colorName of Object.keys(DefaultColorThemes[themeName].colors)) {
			const colorStyleValues = new Set<string>(DefaultColorStyle.values)
			colorStyleValues.add(colorName)
			// @ts-expect-error - values are supposed to be readonly
			DefaultColorStyle.values = Array.from(colorStyleValues)
			// @ts-expect-error - values are supposed to be readonly
			defaultColorNamesSet.add(colorName)
		}
	}
}
