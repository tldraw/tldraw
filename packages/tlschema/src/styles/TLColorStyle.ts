...
/** @public */
export const DefaultColorThemes = atom<Record<string, TLDefaultColorTheme>>(
...
/** @public */
export function getDefaultColorTheme(opts: { colorScheme: string }): TLDefaultColorTheme {
	return DefaultColorThemes.get()[opts.colorScheme]
}
...
const defaultColorNamesSet = atom<Set<string>>(
	'default color names set',
	new Set(defaultColorNames)
)
...
export function isDefaultThemeColor(
	...
	return defaultColorNamesSet.get().has(color as string)
}
...
export function getColorValue(
	...
	if (isDefaultThemeColor(color)) {
		if (variant) {
			return (theme as any).colors[color]?.[variant]
		} else {
			return (theme as any).colors[color]?.solid
		}
	}
...
export function extendDefaultColorTheme(
	updater: (themes: Record<string, TLDefaultColorTheme>) => Record<string, TLDefaultColorTheme>
) {
	transact(() => {
		DefaultColorThemes.update(updater)

		// todo: check that all themes have the same colors
		const themes = Object.values(DefaultColorThemes.get())
		const allColors = new Set<string>(Object.keys(themes[0].colors))
		for (let i = 1; i < themes.length; i++) {
			const theme = themes[i]
			const themeColors = Object.keys(theme.colors)

			// Check that all themes have the same colors
			allColors.forEach((colorName) => {
				if (!themeColors.includes(colorName)) {
					throw new Error(`Theme ${theme.id} is missing color ${colorName}`)
				}
			})
			themeColors.forEach((colorName) => {
				if (!allColors.has(colorName)) {
					throw new Error(`Theme ${theme.id} has color ${colorName} that is not in the first theme`)
				}
			})
		}

		// Update the color names set
		defaultColorNamesSet.update((set) => {
			set.clear()
			allColors.forEach((colorName) => set.add(colorName))
			return set
		})
