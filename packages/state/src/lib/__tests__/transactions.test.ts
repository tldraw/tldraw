...
describe('async tests generated by claude', () => {
	...
	it('should rollback when rollback callback is called', async () => {
		const a = atom('', 0)
		const b = atom('', 0)

		await asyncTransaction(async (rollback) => {
			a.set(1)
			b.set(2)
			await sleep(1)
			rollback()
	...
	it('should handle rollback after multiple async operations', async () => {
		const a = atom('', 0)
		const b = atom('', 0)
	...
	it('should defer effects until async transaction commits', async () => {
		...
		const txPromise = asyncTransaction(async () => {
			a.set(1)
	...
	it('should handle multiple concurrent async transactions', async () => {
		...
		const tx1 = asyncTransaction(async () => {
			a.set(1)
			await sleep(10)
			results.push(a.get())
			return 'tx1'
		...
		const tx2 = asyncTransaction(async () => {
			b.set(2)
			await sleep(5)
			results.push(b.get())
			return 'tx2'
	...
	it('should handle nested async transaction rollback', async () => {
		const a = atom('', 0)
		const b = atom('', 0)

		await asyncTransaction(async (outerRollback) => {
			a.set(1)

			await asyncTransaction(async (innerRollback) => {
				b.set(2)
				innerRollback()
			})

			expect(a.get()).toBe(1) // outer changes should remain
			expect(b.get()).toBe(0) // inner changes should be rolled back

			outerRollback()
		})

		expect(a.get()).toBe(0) // all changes rolled back
		expect(b.get()).toBe(0)
	})
