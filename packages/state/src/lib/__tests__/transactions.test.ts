...
describe('enhanced async tests', () => {
	...
	it('should handle multiple overlapping async processes in same transaction', async () => {
		const a = atom('', 0)
		let processCount = 0

		// Start multiple async processes in the same transaction context
		const processes = Array.from({ length: 3 }, (_, i) =>
			deferAsyncEffects(async () => {
				processCount++
				a.set(i + 1)
				await sleep(5 * (i + 1)) // Different delays
				return i
			})
		)
		...
		// The final value should be from the last process
		expect(a.get()).toBe(3)
	})
})
