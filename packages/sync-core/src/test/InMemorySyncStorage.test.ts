	...
	describe('getChangesSince', () => {
		it('returns puts for records changed after sinceClock', () => {
			...
			storage.transaction((txn) => {
				...
				const puts = Array.from(changes.puts)

				expect(puts.length).toBe(1)
				expect(puts[0].id).toBe(defaultRecords[1].id) // only record with clock 10 > 7
		...
		it('returns deletes for tombstones after sinceClock', () => {
			...
			storage.transaction((txn) => {
				...
				expect(deletes).toEqual(['shape:deleted2']) // only tombstone with clock 12 > 7
			})
		})
		...
		it('returns empty arrays when no changes since clock', () => {
			const storage = new InMemorySyncStorage<TLRecord>({
				snapshot: makeSnapshot(defaultRecords, {
					documents: [
						{ state: defaultRecords[0], lastChangedClock: 5 },
		...
		describe('getChangesSince boundary conditions', () => {
			...
			it('handles negative sinceClock', () => {
				...
				storage.transaction((txn) => {
					...
					expect(Array.from(changes.puts).length).toBe(2)
				})
			})

			it('returns no changes when sinceClock equals current documentClock', () => {
				const storage = new InMemorySyncStorage<TLRecord>({
					snapshot: makeSnapshot(defaultRecords, {
						documents: [
							{ state: defaultRecords[0], lastChangedClock: 5 },
				...
				storage.transaction((txn) => {
					const changes = txn.getChangesSince(10)
					// lastChangedClock > sinceClock, so 10 > 10 is false
					expect(Array.from(changes.puts).length).toBe(0)
				})
			})
