...
export class TLSyncRoom<R extends UnknownRecord, SessionMeta> {
	...
	private removeSession(sessionId: string, fatalReason?: string) {
		...
		if (presence) {
			...
			this._unsafe_broadcastPatch({
				...
				// safe to not downgrade this diff because deletes don't need to be downgraded
				txn: null,
			})
		}
	 ...
	 * Marked as unsafe because it does not downgrade diffs if the txn is null.
	 */
	_unsafe_broadcastPatch({
		diff,
		sourceSessionId,
		txn,
	}: {
		...
		this.sessions.forEach((session) => {
			...
			if (session.requiresDownMigrations && txn) {
				const result = this.migrateDiffForSession(txn, session.serializedSchema, diff)
				if (!result.ok) {
					this.rejectSession(
						session.sessionId,
	...
	private getDiffSince(txn: TLSyncStorageTransaction<R>, sinceClock: number) {
		...
		for (const put of changes.puts) {
			didChange = true
			diff[put.id] = [RecordOpType.Put, put]
		}
		for (const id of changes.deletes) {
			didChange = true
			diff[id] = [RecordOpType.Remove]
		}
		return { diff, didChange, wipeAll: changes.wipeAll }
	}
	...
	private broadcastChanges(txn: TLSyncStorageTransaction<R>) {
		...
		if (didChange) {
			this._unsafe_broadcastPatch({ diff, txn })
		}
	}
	...
	private handleConnectRequest(
		...
		const { documentClock, result } = this.storage.transaction((txn) => {
			this.broadcastChanges(txn)
			const { diff, wipeAll } = this.getDiffSince(txn, message.lastServerClock)
			...
			return {
				...
				hydrationType: wipeAll ? 'wipe_all' : 'wipe_presence',
	...
	private handlePushRequest(
		...
		const { result, documentClock } = this.storage.transaction(
			(
				txn
			): {
				...
				if (docChanges.diff || presenceChanges.diff) {
					...
					for (const otherSession of this.sessions.values()) {
						...
						const downPresence = presenceChanges.diff
							? otherSession.requiresDownMigrations
								? this.migrateDiffForSession(
										txn,
										otherSession.serializedSchema,
										presenceChanges.diff
									)
								: Result.ok(presenceChanges.diff)
							: undefined

						const downDoc = docChanges.diff
							? otherSession.requiresDownMigrations
								? this.migrateDiffForSession(txn, otherSession.serializedSchema, docChanges.diff)
								: Result.ok(docChanges.diff)
							: undefined
