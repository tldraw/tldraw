...
export class InMemorySyncStorage<R extends UnknownRecord> implements TLSyncStorage<R> {
    ...
    // (undocumented)
    transaction<T>(callback: (txn: TLSyncStorageTransaction<R>) => T, opts?: TLSyncStorageTransactionOptions): TLSyncStorageTransactionResult<T>;
}

// @public
export function loadSnapshotIntoStorage<R extends UnknownRecord>(txn: TLSyncStorageTransaction<R>, schema: StoreSchema<R, any>, snapshot: RoomSnapshot): void;
...
export class TLSyncRoom<R extends UnknownRecord, SessionMeta> {
    ...
    _unsafe_broadcastPatch({ diff, sourceSessionId, txn, }: {
        diff: NetworkDiff<R>;
        sourceSessionId?: string;
        txn: null | TLSyncStorageTransaction<R>;
    }): this;
}
