...
export function getStraightArrowInfo(
	...
	if (
		...
		if (endShapeInfo.didIntersect && !startShapeInfo.didIntersect) {
			...
			if (startShapeInfo.isClosed) {
				a.setTo(b.clone().add(uAB.clone().mul(MIN_ARROW_LENGTH * shape.scale)))
			}
			...
			if (endShapeInfo.isClosed) {
				b.setTo(a.clone().sub(uAB.clone().mul(MIN_ARROW_LENGTH * shape.scale)))
			}
		}
	}
	...
	if (!isSelfIntersection) {
		if (
			...
			strokeOffsetA =
				...
				('size' in startShapeInfo.shape.props
					...
					: 0)
			offsetA = (BOUND_ARROW_OFFSET + strokeOffsetA) * shape.scale
			minLength += strokeOffsetA * shape.scale
		}
		...
		if (
			...
			offsetB = (BOUND_ARROW_OFFSET + strokeOffsetB) * shape.scale
			minLength += strokeOffsetB * shape.scale
		}
	}
	...
	if (didFlip) {
		if (startShapeInfo && endShapeInfo) {
			...
			b.setTo(Vec.Add(a, u.clone().mul(-MIN_ARROW_LENGTH * shape.scale)))
		}
