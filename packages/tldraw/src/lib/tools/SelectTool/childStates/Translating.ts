...
export function moveShapesToPoint({
	...
	const isSnapping = editor.user.getIsSnapMode() ? !inputs.getCtrlKey() : inputs.getCtrlKey()
	let snappedToPit = false
	if (isSnapping && Vec.Len(editor.inputs.getPointerVelocity()) < 0.5) {
		// snapping
	...
	if (isGridMode && !inputs.getCtrlKey() && !snappedToPit && snapIndicators.length === 0) {
