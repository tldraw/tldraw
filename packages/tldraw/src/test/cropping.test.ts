...
describe('When in the select.pointing_crop_handle state', () => {
	it('moving the pointer should transition to select.cropping', () => {
		editor
			...
			.select(ids.imageB)
			.keyDown('Control')
			.pointerDown(500, 600, { target: 'selection', handle: 'bottom_left' })
			.expectToBeIn('select.pointing_crop_handle')
			.pointerMove(510, 590)
			.expectToBeIn('select.resizing_crop')
	})
...
describe('When in the select.cropping state', () => {
	...
	it('escape / cancel should revert the change and transition to crop.idle when that is the history state', () => {
		...
		editor
			...
			.expectToBeIn('select.cropping.pointing_crop_handle')
			.pointerMove(510, 590)
			.expectToBeIn('select.cropping.resizing_crop')
			.cancel()
			.expectToBeIn('select.cropping.idle')
...
describe('When cropping...', () => {
	...
	it('Crop the image from the top right', () => {
		...
		editor
			.select(ids.imageA)
			.keyDown('Control')
			.pointerDown(imageX + imageWidth, imageY, {
				target: 'selection',
				handle: 'top_right',
			})
			.expectToBeIn('select.pointing_crop_handle')
	...
	it('Crop the image from the bottom left', () => {
		...
		editor
			.select(ids.imageA)
			.keyDown('Control')
			.pointerDown(imageX, imageY + imageHeight, {
				target: 'selection',
				handle: 'bottom_left',
			})
			.expectToBeIn('select.pointing_crop_handle')
	...
	it('Crop the image from the bottom right', () => {
		...
		editor
			.select(ids.imageA)
			.keyDown('Control')
			.pointerDown(imageX + imageWidth, imageY + imageHeight, {
				target: 'selection',
				handle: 'bottom_right',
			})
			.expectToBeIn('select.pointing_crop_handle')
	...
	it('Crop the image from the left', () => {
		...
		editor
			.select(ids.imageA)
			.keyDown('Control')
			.pointerDown(imageX, imageY + imageHeight / 2, {
				target: 'selection',
				handle: 'left',
			})
			.expectToBeIn('select.pointing_crop_handle')
