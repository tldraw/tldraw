...
it('does not zoom past max', () => {
	...
	editor.camera.zoomToSelection()
	expect(editor.camera.getZoom()).toBe(1) // double check again when we're zooming in hard
})
...
it('does not zoom to selection when camera is frozen', () => {
	const cameraBefore = { ...editor.camera.get() }
	editor.updateInstanceState({ canMoveCamera: false })
	editor.setSelectedShapes([ids.box1, ids.box2])
	editor.camera.zoomToSelection()
	expect(editor.camera.get()).toMatchObject(cameraBefore)
})
