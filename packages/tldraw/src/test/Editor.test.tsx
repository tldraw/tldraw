...
describe('Editor.TickManager', () => {
	it('Does not produce NaN values when elapsed is 0', () => {
		...
		const tick = (ms: number) => {
			// @ts-ignore
			editor.tickManager.updatePointerVelocity(ms)
		}

		// 1. pointer velocity should be 0 when there is no movement
		expect(editor.inputs.getPointerVelocity()).toCloselyMatchObject({ x: 0, y: 0 })
		...
		tick(16)
		expect(editor.inputs.getPointerVelocity()).toCloselyMatchObject({ x: 0.3125, y: 0.3125 })

		// 4. let's do it again, it should be updated again. move, tick, measure
		editor.pointerMove(20, 20)
		tick(16)
		expect(editor.inputs.getPointerVelocity()).toCloselyMatchObject({
			x: 0.46875,
			y: 0.46875,
		})
