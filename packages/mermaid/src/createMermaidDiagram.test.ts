...
describe('createMermaidDiagram', () => {
	...
	it('throws MermaidDiagramError with type "unsupported" when no callback is provided', async () => {
		const { editor } = createMockEditor()

		await expect(
			createMermaidDiagram(editor, 'pie\n  "Dogs" : 386', {
				blueprintRender: { position: { x: 0, y: 0 }, centerOnPosition: false },
			})
		).rejects.toThrow(MermaidDiagramError)

		await expect(createMermaidDiagram(editor, 'pie\n  "Dogs" : 386')).rejects.toMatchObject({
			type: 'unsupported',
			diagramType: 'pie',
		})
	})
