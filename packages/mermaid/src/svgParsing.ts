...
export type NodeIdParser = (domId: string) => string
export type EdgeIdParser = (dataId: string) => { start: string; end: string } | null
...
export function parseClustersFromSvg(root: Element, selector: string): Map<string, ParsedCluster> {
	...
	for (const g of root.querySelectorAll(selector)) {
		...
		out.set(id, {
			id,
			topLeft: { x: ancestor.x + rx, y: ancestor.y + ry },
			width: w,
			height: h,
		})
	}
...
export function computeBounds(
	...
	let minX = Infinity,
		...
		maxY = -Infinity
	for (const [, n] of nodes) {
		minX = Math.min(minX, n.center.x - n.width / 2)
		minY = Math.min(minY, n.center.y - n.height / 2)
		maxX = Math.max(maxX, n.center.x + n.width / 2)
		maxY = Math.max(maxY, n.center.y + n.height / 2)
	}
	for (const [, c] of clusters) {
		minX = Math.min(minX, c.topLeft.x)
		minY = Math.min(minY, c.topLeft.y)
		maxX = Math.max(maxX, c.topLeft.x + c.width)
		maxY = Math.max(maxY, c.topLeft.y + c.height)
	}
