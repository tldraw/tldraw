...
function flattenMindmapTree(node: MindmapNode, parent: MindmapNode | undefined, out: FlatNode[]) {
	out.push({
		id: String(node.id),
		label: node.descr,
		type: node.type,
	...
	for (const child of node.children) {
		flattenMindmapTree(child, node, out)
	}
}
...
function parseMindmapNodeId(domId: string): string {
	return parseDomId(domId, NODE_ID)
}
...
export function mindmapToBlueprint(
	...
	for (const el of svgRoot.querySelectorAll('.node')) {
		...
		if (parsed) nodeColors.set(parseMindmapNodeId(el.getAttribute('id') || ''), parsed.color)
	}
	...
	for (const flatNode of flatNodes) {
		const svgNode = layout.nodes.get(flatNode.id)
		if (!svgNode) continue
		...
		if (flatNode.parentId) {
			edges.push({
				...
				arrowheadEnd: 'none',
				arrowheadStart: 'none',
				size: getEdgeSizeForLevel(flatNode.parentLevel),
				color,
			})
		}
	}
