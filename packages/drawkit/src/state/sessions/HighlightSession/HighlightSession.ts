...
import { HighlightShape, SessionType, TDStatus, TldrawCommand, TldrawPatch } from '~types'

export class HighlightSession extends BaseSession {
  type = SessionType.Highlight
  performanceMode = undefined
  status = TDStatus.Creating
  topLeft: number[]
  points: number[][]
  initialShape: HighlightShape
  ...
  constructor(app: TldrawApp, id: string) {
    ...
    this.shapeId = id
    this.initialShape = this.app.getShape<HighlightShape>(id)
  ...
  complete = (): TldrawPatch | TldrawCommand | undefined => {
    const { shapeId } = this
    const pageId = this.app.currentPageId
    const shape = this.app.getShape<HighlightShape>(shapeId)
    return {
      id: 'create_highlight',
      before: {
        document: {
