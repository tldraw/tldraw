 ...
 * the record store.
 */
export class SyncStoreQueries<R extends UnknownRecord> {
	history: Atom<number, RecordsDiff<R>>
	constructor(private readonly store: SyncStore<R, any>) {
	...
	>(typeName: TypeName, property: Property): RSIndex<Extract<R, { typeName: TypeName }>, Property> {
		...
		const fromScratch = () => {
			...
			for (const [_id, record] of this.store.entries()) {
	...
	record<TypeName extends R['typeName']>(
		...
		return computed<S | undefined>(name, () => {
			for (const id of ids.value) {
				return this.store.get(id as string) as S
			}
			return undefined
		})
	}
	...
	records<TypeName extends R['typeName']>(
		...
		return computed<S[]>(name, () => {
			return [...ids.value].map((id) => {
				const record = this.store.get(id as string)
				if (!record) {
		...
		const fromScratch = () => {
			...
			if (Object.keys(query).length === 0) {
				return new Set<IdOf<S>>(
					this.store.entries().flatMap(([_id, r]) => {
	...
	exec<TypeName extends R['typeName']>(
		...
		return [...ids].map((id) => this.store.get(id) as any)
	}
}
