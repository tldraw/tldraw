...
export class StoreSchema<R extends UnknownRecord, P = unknown> {
	...
	private constructor(
		...
		this.migrations = Object.fromEntries(
			Object.entries(options.migrations ?? {}).map(([k, v]) => [k, v.sequence])
		)
	}
	...
	public getMigrationsSince(persistedSchema: SerializedSchema): Result<Migration[], string> {
		...
		// find any shared sequences
		const sharedSequences = Object.keys(schema.sequences).filter(
			(sequenceId) => this.migrations[sequenceId]
		)
		...
		for (const sequenceId of sharedSequences) {
			const theirVersionNumber = schema.sequences[sequenceId]
			// Special case for legacy situations where there was no schema.
			// -1s should only be generated by `serializeEarliestVersion`
			if (theirVersionNumber === -1) {
				result.push(...this.migrations[sequenceId])
				continue
			}
			const theirVersionId = `${sequenceId}/${schema.sequences[sequenceId]}`
			const idx = this.migrations[sequenceId].findIndex((m) => m.id === theirVersionId)
			// todo: better error handling
			if (idx === -1) return Result.err('Incompatible schema?')
			result.push(...this.migrations[sequenceId].slice(idx + 1))
		}
		...
		return {
			...
			sequences: Object.fromEntries(
				Object.entries(this.migrations).map(([sequenceId, sequence]) => [
					sequenceId,
					parseMigrationId(sequence.at(-1)!.id).version,
				])
			),
		}
	}
