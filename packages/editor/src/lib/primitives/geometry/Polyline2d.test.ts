...
describe('Polyline2d.hitTestLineSegment', () => {
	...
	it('measures the distance from the edges, not only the vertices', () => {
		// the segment is 10 units from the middle of the top edge but 41 from the nearest vertex (#10556)
		expect(polyline().hitTestLineSegment(new Vec(40, 10), new Vec(60, 10), 9)).toBe(false)
		expect(polyline().hitTestLineSegment(new Vec(40, 10), new Vec(60, 10), 10)).toBe(true)
	})
})
