...
import { startCommentThread, useCommentThreads, useThreadComments } from '../hooks'
...
export function CommentsOnCanvas() {
	...
	const { user, components } = useComments()
	...
	const ThreadPinComponent = components.ThreadPin ?? ThreadPin
	const ThreadComposerComponent = components.ThreadComposer ?? ThreadComposer

	return (
		<>
			{threads.map((thread) => (
				<ThreadPinComponent key={thread.id} thread={thread} />
			))}
			{selectedShapeId && authorId && <ThreadComposerComponent shapeId={selectedShapeId} />}
		</>
	)
}
...
function ThreadPin({ thread }: { thread: TLCommentThread }) {
	const editor = useEditor()
...
function ThreadComposer({ shapeId }: { shapeId: TLShapeId }) {
	const editor = useEditor()
	const { user } = useComments()
	...
	const submit = () => {
		...
		startCommentThread(editor, { anchor: { type: 'shape', shapeId }, body: trimmed, user })
		setText('')
	}
