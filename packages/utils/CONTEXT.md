...
class WeakCache<T extends object, U> {
  ...
  const cache = new WeakCache<Shape, BoundingBox>()
  const bbox = cache.get(shape, (s) => computeBoundingBox(s)) // Computed once per shape
}
```
...
describe('ExecutionQueue', () => {
  it('executes tasks sequentially', async () => {
    ...
    await Promise.all([
      queue.push(() => { results.push(1); return 1 }),
      queue.push(() => { results.push(2); return 2 }),
      queue.push(() => { results.push(3); return 3 })
    ])
