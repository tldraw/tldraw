#!/bin/bash
# Update (or create) the draft GitHub release for the next release using the
...
#
# Usage: ./update-draft-release.sh <tldraw-repo-dir> [next-version]
#
# next-version overrides the target tag (with or without a leading `v`). Pass it
# for a major release or any override, so the draft name matches the version
# publish-new.ts will resolve. When omitted, the target defaults to the next
# minor of last_version — the common case.
#
...
if [ -z "$1" ]; then
  echo "Usage: $0 <tldraw-repo-dir> [next-version]" >&2
  echo "Example: $0 /tmp/tldraw          # minor bump of last_version" >&2
  echo "Example: $0 /tmp/tldraw v6.0.0   # explicit target (major/override)" >&2
  exit 1
fi
...
if [ -n "$2" ]; then
  # Explicit target — normalize to a leading `v`. Use this for major releases or
  # overrides, where publish-new.ts resolves the next version from NPM rather than
  # a minor bump (e.g. --bump major -> v6.0.0).
  next_tag="v$(echo "$2" | sed 's/^v//')"
else
