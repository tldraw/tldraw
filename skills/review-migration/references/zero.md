...
- Column adds and the default rule
...
| Mass update of a published table | Not in a migration: batch it outside, or add a column and let the backfill carry it | One transaction replicates as one change-log write; ~980k rows blocked all replication for 15 minutes |
...
Incident 2026-06-18 (P0, about an hour of stalled sync): migration 037 added `inviteLinkEnabled BOOLEAN NOT NULL DEFAULT true` to `group`, and the client code that read it shipped in the same deploy. The backfill took 52 minutes, during which every query touching the column errored. In the same deploy, migration 036 updated every row of `group` (~980k rows), which replicated as one 15.5-minute change-log transaction and blocked all replication to clients while it ran. Rocicorp's guidance, which still stands:
...
The July 2026 comments release shipped five migrations (040 to 044, including a `file_visitor` backfilled from `file_state`) in a migrations-only deploy, waited for the backfill, then shipped the code a deploy later. Done-checks, verified against Zero 1.8 source:
...
- Replication health during a backfill: `zero_replication_total_lag_millisecond`, WAL retained per slot (`pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn)` from `pg_replication_slots`), both on the Zero health dashboard. The Notion page "Running zero" is the ops runbook.
