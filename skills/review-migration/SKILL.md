...
- **`-- no-transaction`** as the first line (from #10576) runs the file outside the transaction, statement by statement. Only for `CREATE INDEX CONCURRENTLY`. Its ledger row is written as soon as it succeeds, the dry run skips it, and a failed concurrent index build leaves an INVALID index that a retry with `IF NOT EXISTS` keeps: drop it first.
...
- **Column add.** On production every column added to a published table is backfilled: Zero re-streams the whole table and the column is invisible to clients and the sync-worker until that finishes on every replica (52 minutes for ~980k rows in June 2026). The default makes no difference here: Zero's in-place optimisation for simple defaults only works on the event-trigger path, and Supabase delivers our schema changes through the manual `update_schemas()` hook. Locally it looks instant, which is the trap. Code that reads the column ships a deploy after the backfill is confirmed done, never in the same deploy as the migration; that ordering mistake, not the column itself, was the 2026-06-18 outage (migration 037).
- **Mass update.** A migration that updates every row of a published table replicates as one change-log transaction and blocks all replication to clients while Zero writes it through (~980k rows of `group` in migration 036 took 15 minutes). Add a column and let the backfill carry the value instead, or batch the update outside the migration.
...
- **New table read by clients.** It is invisible until every replica (RM plus every VS machine) has backfilled it, and all backfills share one queue. Ship the migrations alone, confirm the backfill is done, then ship the code. The July 2026 comments release did this deliberately after 037.
...
Read `references/locks.md` for lock modes, the writer-order recipe, and the deadlock story. The checks:
...
- The Zero visibility window: none, a column backfill, or a table backfill, sized from the production row count, and how it will be confirmed done before dependent code ships (`SELECT * FROM "zero_0/cdc"."backfilling"` empty).
- Off-peak. Connected clients bottom out between 23:00 and 05:00 UTC on weekdays and Sunday early morning; the pipeline reset still hits every open tab.
- Rerun on deadlock: `gh run rerun <deploy-run-id> --failed`. The migrate step is first, so a failure there has rolled nothing else.
- How to see it committed on production: `increase(zero_sync_pipeline_resets_total{deployment_environment="production", reason="schema-change"}[15m])` goes non-zero; for a new table, the upstream `backfilling` table empties.
- If it goes out as a hotfix: the cherry-pick onto `hotfixes` conflicts whenever `hotfixes` lacks a main-only change to the same file (`migrate.ts` after a refactor, say). Recovery is a manual `hotfix/dotcom-<pr>` branch off `origin/hotfixes`, resolve, push, PR to `hotfixes`, human merge.
