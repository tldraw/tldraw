...
- **New table read by clients.** It is invisible until every replica (RM plus every VS machine) has backfilled it, and all backfills share one queue. Ship the migrations alone, confirm the backfill is done, then ship the code.
