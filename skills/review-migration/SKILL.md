...
Postgres documents which lock each statement takes and how deadlocks resolve (https://www.postgresql.org/docs/current/explicit-locking.html and the per-form notes in https://www.postgresql.org/docs/current/sql-altertable.html); `references/locks.md` has only what is specific to us: how the runner holds locks, the writer-order recipe, the reader deadlock and the rerun, and which statements need more than one release here. The checks:

- **Which locks, how long.** Look up each statement's lock in the docs above; most DDL here is ACCESS EXCLUSIVE, and every lock is held until the runner commits after the last pending file, not until the statement ends. Writes and reads to those tables queue for the whole remaining transaction. List the tables and expect the PR body to.
