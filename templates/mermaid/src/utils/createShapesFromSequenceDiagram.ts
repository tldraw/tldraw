...
export function createShapesFromSequenceDiagram(
	...
	for (let pIndex = 0; pIndex < diagram.participants.length; pIndex++) {
		const participant = diagram.participants[pIndex]
	...
	for (let msgIndex = 0; msgIndex < diagram.messages.length; msgIndex++) {
		const message = diagram.messages[msgIndex]
	...
	for (const loop of diagram.loops) {
		...
		const endY =
			loop.endIndex < messageYPositions.length ? messageYPositions[loop.endIndex] : currentY
		...
		editor.createShape<TLGeoShape>({
			...
			meta: {
				diagramType: 'sequenceDiagram',
				loopData: { type: loop.type, label: loop.label || '' },
			},
		})
	}
	...
	for (let i = 0; i < diagram.notes.length; i++) {
		...
		editor.createShape<TLGeoShape>({
			...
			meta: {
				diagramType: 'sequenceDiagram',
				noteData: { position: note.position, participants: note.participants, text: note.text },
			},
		})
	}
}
