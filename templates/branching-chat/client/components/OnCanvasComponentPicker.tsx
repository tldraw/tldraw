...
import { MessageNode } from '../nodes/types/MessageNode'
...
export interface OnCanvasComponentPickerState {
	...
	onPick: (nodeType: NodeType, position: VecModel, direction: 'horizontal' | 'vertical') => void
	onClose: () => void
}
...
export function OnCanvasComponentPicker() {
	...
	return (
		<OnCanvasComponentPickerDialog onClose={onClose}>
			<TldrawUiMenuGroup id="chat">
				<OnCanvasComponentPickerItem
					definition={MessageNode}
					onClose={onClose}
					direction="vertical"
				/>
			</TldrawUiMenuGroup>
	...
	return (
		<TldrawUiButton
			...
			onClick={() => {
				...
				state.onPick(definition.getDefault(), terminalInPageSpace, direction)
