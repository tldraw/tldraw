...
// Assets (generated backgrounds and any user-dropped media) are stored in the
// R2 bucket under the /uploads path.
...
// when an asset is uploaded, we store it in the bucket. we only allow image and video assets.
...
// when an asset is downloaded, we retrieve it from the bucket. we also cache the response for performance.
