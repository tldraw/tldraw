...
export const cspDev = Object.keys(cspDirectives)
	...
	.map((directive) => {
		...
		// Parallel worktrees: the worker/zero ports above are hard-coded to the natural block
		// (8788/8789/4848). A parallel worktree runs on an offset block, so allow any localhost port in
		// dev for connect-src — otherwise the browser blocks the offset workers/zero.
