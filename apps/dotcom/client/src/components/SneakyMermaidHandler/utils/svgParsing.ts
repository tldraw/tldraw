...
// ---------------------------------------------------------------------------
// High-level SVG parsing
// ---------------------------------------------------------------------------

export function mountSvg(svgString: string): { root: Element; cleanup(): void } {
	const container = document.createElement('div')
	container.style.cssText = 'position:fixed;left:-9999px;top:-9999px;overflow:hidden'
	document.body.appendChild(container)
	container.innerHTML = svgString
	const liveSvg = container.querySelector('svg')
	const root: Element =
		liveSvg || new DOMParser().parseFromString(svgString, 'image/svg+xml').documentElement
	return { root, cleanup: () => container.remove() }
}
