/**
 * FairyHorizontalBarChart - Standalone Horizontal Bar Chart Implementation
...
interface ChartOptions {
	...
	height?: number
	colors?: string[]
	/** Individual colors for each bar (overrides dataset colors) */
	barColors?: string[]
	showLegend?: boolean
	showTooltip?: boolean
	barSpacing?: number
...
export class FairyHorizontalBarChart {
	...
	private createSVGContainer(): void {
		this.svg = createSVGElement('svg', {
			className: 'fairy-horizontal-bar-chart',
			width: this.width,
			height: this.height,
			styles: {
				overflow: 'visible',
	 ...
	 * Labels are positioned to align with the center of each bar
	 */
	private renderYAxis(group: SVGGElement): void {
		this.yAxis.positions.forEach((pos, i) => {
			// Labels are centered on the slot position (same as bar center)
	 ...
	 * Supports per-bar colors via options.barColors array
	 */
	private renderBars(group: SVGGElement): void {
		const numLabels = this.data.labels.length
		const unitHeight = this.chartHeight / numLabels
		const barsHeight = unitHeight * (1 - this.options.barSpacing!)
		const barHeight = barsHeight // Single bar per label when using barColors

		// Use first dataset's values (single dataset mode with per-bar colors)
		...
		dataset.values.forEach((value, i) => {
			...
			// Calculate bar position (centered on the slot)
			...
			// Get color: prefer barColors array, then dataset color, then default colors
			const color =
				this.options.barColors?.[i] || dataset.color || this.colors[i % this.colors.length]
	...
	private renderLegend(group: SVGGElement): void {
		...
		this.data.datasets.forEach((dataset, idx) => {
			...
			const label = makeText('legend-label', x + 16, legendY, truncateString(dataset.name, 15), {
				fontSize: 11,
				fill: '#666',
				dy: 9,
			})
	...
	private showTooltipAt(e: MouseEvent, relY: number): void {
		// Find closest bar by Y position
		const numLabels = this.data.labels.length
		const unitHeight = this.chartHeight / numLabels
	...
	public update(data: ChartData, barColors?: string[]): void {
		this.data = deepClone(data)
		// Update barColors if provided
		if (barColors !== undefined) {
			this.options.barColors = barColors
		}
		this.render()
	}
