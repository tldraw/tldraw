 ...
 */
function getLintKey(lint: FairyCanvasLint): string {
...
export class FairyAgentLintManager extends BaseFairyAgentManager {
	 ...
	 */
	detectCanvasLints(shapes: TLShape[]): FairyCanvasLint[] {
	...
	/**
	 * Check if a text shape intersects the edge of a geo shape.
	 */
	private textShapeIntersectsGeoEdge(textShape: TLShape, geoShape: TLShape): boolean {
		const { editor } = this.agent
