import { getFocusedShapeSchemaNames } from '@tldraw/fairy-shared'
import {
	TldrawUiButton,
	TldrawUiButtonLabel,
	TldrawUiDialogBody,
...
import type { TldrawFairyAgent } from './fairy-agent/agent/TldrawFairyAgent'

export function FairyDebugModal({
	agents,
	onClose,
}: {
	agents: TldrawFairyAgent[]
	onClose(): void
}) {
...
// ------- System prompt flag debugging -------

function handleDebugSystemPrompt() {
	// Randomly enable/disable each flag with 50% probability
	const randomFlags: SystemPromptFlags = {
		// Communication
		hasMessage: Math.random() > 0.5,

		// Planning
		hasThink: Math.random() > 0.5,
		hasReview: Math.random() > 0.5,
		hasAddDetail: Math.random() > 0.5,
		hasUpdateTodoList: Math.random() > 0.5,
		hasFlyToBounds: Math.random() > 0.5,
		hasNoteToSelf: Math.random() > 0.5,

		// Individual shapes
		hasCreate: Math.random() > 0.5,
		hasDelete: Math.random() > 0.5,
		hasUpdate: Math.random() > 0.5,
		hasLabel: Math.random() > 0.5,
		hasMove: Math.random() > 0.5,

		// Groups of shapes
		hasPlace: Math.random() > 0.5,
		hasBringToFront: Math.random() > 0.5,
		hasSendToBack: Math.random() > 0.5,
		hasRotate: Math.random() > 0.5,
		hasResize: Math.random() > 0.5,
		hasAlign: Math.random() > 0.5,
		hasDistribute: Math.random() > 0.5,
		hasStack: Math.random() > 0.5,
		hasClear: Math.random() > 0.5,

		// Drawing
		hasPen: Math.random() > 0.5,

		// Internal (required)
		hasUnknown: Math.random() > 0.5,

		// Model
		hasSystemPart: Math.random() > 0.5,
		hasModelNamePart: Math.random() > 0.5,

		// Request
		hasMessagesPart: Math.random() > 0.5,
		hasDataPart: Math.random() > 0.5,
		hasContextItemsPart: Math.random() > 0.5,

		// Viewport
		hasScreenshotPart: Math.random() > 0.5,
		hasViewportBoundsPart: Math.random() > 0.5,

		// Shapes
		hasBlurryShapesPart: Math.random() > 0.5,
		hasPeripheralShapesPart: Math.random() > 0.5,
		hasSelectedShapesPart: Math.random() > 0.5,

		// History
		hasChatHistoryPart: Math.random() > 0.5,
		hasUserActionHistoryPart: Math.random() > 0.5,
		hasTodoListPart: Math.random() > 0.5,

		// Metadata
		hasTimePart: Math.random() > 0.5,
	}

	debugPrintSystemPrompt(randomFlags)
}

const shapeTypeNames = getFocusedShapeSchemaNames()

interface SystemPromptFlags {
	// Action flags - Communication
	hasMessage: boolean

	// Action flags - Planning
	hasThink: boolean
	hasReview: boolean
	hasAddDetail: boolean
	hasUpdateTodoList: boolean
	hasFlyToBounds: boolean
	hasNoteToSelf: boolean

	// Action flags - Individual shapes
	hasCreate: boolean
	hasDelete: boolean
	hasUpdate: boolean
	hasLabel: boolean
	...
	// Action flags - Drawing
	hasPen: boolean

	// Action flags - Internal (required)
	hasUnknown: boolean

	// Part flags - Model
	hasSystemPart: boolean
	hasModelNamePart: boolean

	// Part flags - Request
	hasMessagesPart: boolean
	hasDataPart: boolean
	hasContextItemsPart: boolean

	// Part flags - Viewport
	hasScreenshotPart: boolean
	hasViewportBoundsPart: boolean

	// Part flags - Shapes
	hasBlurryShapesPart: boolean
	hasPeripheralShapesPart: boolean
	hasSelectedShapesPart: boolean

	// Part flags - History
	hasChatHistoryPart: boolean
	hasUserActionHistoryPart: boolean
	hasTodoListPart: boolean

	// Part flags - Metadata
	hasTimePart: boolean
}

function getSystemPromptFromFlags(flags: SystemPromptFlags) {
...
function debugPrintSystemPrompt(flags: SystemPromptFlags) {
	console.warn('=== SYSTEM PROMPT FLAGS ===')
	console.warn(JSON.stringify(flags, null, 2))
	console.warn('\n=== SYSTEM PROMPT ===')
	console.warn(getSystemPromptFromFlags(flags))
	console.warn('=== END SYSTEM PROMPT ===')
	console.warn(
		"this doesn't pull from the SystemPromptPartUtil, make sure the system prompt in the FairyDebugModal is up to date"
	)
}
