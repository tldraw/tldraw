...
3. The worker resolves the requested page from the board snapshot and measures its shapes through a render (`measurePageShapes` — clustering needs real geometry and text, which only an editor can produce), resolves the cluster ids against the resulting clusters, and builds an R2 cache key from board identity, a content version, the fixed 1200x630 output size, theme, and a digest of the sorted shape ids (`mcp/{kind}/{slug}/{version}/1200x630/{theme}/shapes-{digest}.png`), with the page name in object metadata. The version is the file's `lastPublished` for published boards and the persisted room snapshot's R2 etag for shared files, so republishing or editing rotates every key. A cache hit in the `MCP_DATA_BUCKET` bucket returns without capturing again — though the measure render is already spent by then, since the key cannot be built before the cluster ids are resolved.
...
- Per user: ~10 Browser Run-spending calls per minute per tool family (`user-shot:`, `user-cluster:`, and `user-info:` on `MCP_SCREENSHOT_RATE_LIMITER`) — cluster screenshots, page info, and cluster info each hold their own budget.
...
Per-board is deliberately far tighter than per-user: a caller gets 10 captures a minute, but no single board may absorb more than 2 of them. Because captures are counted only on cache misses, this does not bound the usual "screenshot several clusters of one board" flow — a repeated capture of the same shapes is a cache hit.
...
Only the calls that actually spend Browser Run are limited. `get_board_info` is not: it resolves a board and reads its snapshot, which is the same work the ordinary board routes already do for that caller. The clustering tools are limited even though they read as "info" calls — they measure the page in a render before they can group anything, which costs the same as a screenshot. The OG image route's per-board limit (`og-board:`) is gone too, along with the route's enqueue — see "OG images (queue-backed async rendering)" above.
...
- `MCP_SERVER_URL` - the MCP server's public URL and its OAuth resource identifier (RFC 8707): what protected resource metadata advertises and what an access token's `aud` must name. Set in `wrangler.toml` for dev, staging, and production; previews have no entry, so `deploy-dotcom.ts` injects `https://${previewId}-preview-deploy.tldraw.com/api/app/mcp` as a deploy var. Left unset it would be derived from the request's own `Host` header, which is acceptable for local dev and tests only — see `getMcpResourceUrl`.
- `MCP_REQUIRE_TOKEN_AUDIENCE` - whether a token whose `aud` does not name `MCP_SERVER_URL` is refused (unset, and always in production) or merely logged (`"false"`, currently dev, staging, and previews). Temporary, while it is not yet known whether the Clerk OAuth instance stamps the resource indicator: the worker logs every token it would have refused, so staging's logs settle that question before production enforces it. Set in three places — `[env.dev.vars]`, `[env.staging.vars]`, and the preview deploy vars in `deploy-dotcom.ts` — and all three go away together once Clerk is confirmed to stamp `aud`. See `isMcpTokenAudienceRequired` in `mcpAuth.ts`.
...
- **Keyed per capture** (`render-tokens/{kind}/{slug}/{surface}[/{theme}/{pageId}]`), not per token, so records overwrite in place and the space stays bounded by content rather than by traffic. Nothing accumulates, so there is no lifecycle rule to add, and none must ever be added to this bucket.

  What counts as one capture differs by surface, because their concurrency does. The OG pipeline is single-flighted per board by the `.pending` marker, so its renders never overlap and one key per board is right — a board's newest mint invalidating an older in-flight token is then intended, a fresher render superseding one already running.

  The MCP tool has no pending marker, and its per-board limiter deliberately allows two cache-missing captures a minute, so **two captures of different pages of one board would share a per-board key and invalidate each other** — as would an edit-triggered render landing during a capture. The loser fails its snapshot fetch with a 403, which surfaces as a generic `browser_failed`, indistinguishable from a real browser crash. That is why the key carries the surface, and for MCP the page and theme too: authenticating the endpoints is what made it mint `render`, and namespacing the key was the prerequisite for doing so.
...
- **Tokens must name this resource.** `MCP_SERVER_URL` is the RFC 8707 resource identifier, and a token's `aud` must name it — checked explicitly against the verified payload (`namesResource` in `mcpAuth.ts`, on normalized URLs), so a token a user granted to some other MCP server cannot be replayed here. `verifyToken`'s `audience` option is deliberately not used: it compares only when a token carries an `aud` at all (so alone it would accept a Clerk _session_ JWT), and a present-but-wrong `aud` would fail inside verification, upstream of the diagnostic log and the `MCP_REQUIRE_TOKEN_AUDIENCE` escape hatch (see Configuration). The var is configured per environment rather than derived from the request, because the derivation reads the `Host` header — a forged one would otherwise move both the advertised metadata and the audience we check against.
- **The supported versions are `2026-07-28` and `2025-11-25`, one per era, and everything older was dropped.** Dropping `2024-11-05` is what made any of this possible: MCP had no authorization flow before `2025-03-26`, so a client held to the old version has no conformant way to obtain the token every request now needs. `2025-06-18` and `2025-03-26` went with it, which keeps the matrix to one revision per era — `initialize` offers `2025-11-25` to a client asking for either, and the spec's own mechanism takes it from there. That does cost clients which can authenticate but cannot follow an offered version, so it is worth weighing against how many still sit on those revisions. See "Protocol versions" below for how the two eras are served.
...
**One thing to confirm before this is switched on.** Token verification uses `verifyToken` from `@clerk/backend`, which validates a **JWT** against the instance's JWKS. If the Clerk OAuth authorization server is configured to issue opaque access tokens instead, they cannot be verified this way — that needs `idPOAuthAccessToken.verifySecret`, which arrived in `@clerk/backend` v2, and this worker pins 1.23.7, where there is no OAuth token API at all. Check the token format when the Clerk instance is set up; if it is opaque, the SDK upgrade is a prerequisite and it touches every other auth path in the worker.

Client registration is the other thing to configure. Clients identify themselves with Client ID Metadata Documents (CIMD): a client's id is an HTTPS URL to a metadata document its vendor hosts, so there is no registration endpoint. Clerk supports this (beta, enabled per account) through **Advertise CIMD support** and **Only allow pre-registered clients to connect**, turned on together, with the clients we verify allowlisted. We don't support dynamic client registration because of security concerns; a client that requires it isn't supported until it updates. [`mcp-server-auth.md`](./mcp-server-auth.md) has the reasoning.
