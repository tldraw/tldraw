...
- **Tokens must name this resource.** `MCP_SERVER_URL` is the RFC 8707 resource identifier, passed to `verifyToken` as `audience`, so a token a user granted to some other MCP server cannot be replayed here. It is configured per environment rather than derived from the request, because the derivation reads the `Host` header — a forged one would otherwise move both the advertised metadata and the audience we check against.
