...
**The shared read has no index yet, deliberately.** Its access key is on `group_file` and its sort key is on `file`, so nothing spans the two — it fetches every guest link the caller has and sorts them. Measured at production's shape against the heaviest guest-link count (233): 879 buffers, against **89 buffers and 22 rows** with an index on `group_file("groupId", "createdAt" DESC)`. Both are under two milliseconds, and the cost only stops scaling with link count once the index exists.
