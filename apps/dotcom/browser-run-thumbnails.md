...
`GET /app/file/:boardId/thumbnail` (`getBoardThumbnail.ts`, public path `/api/app/file/:boardId/thumbnail`) is a third way a stored thumbnail reaches someone — no Browser Run, and no public gate. A Clerk session (cookie, or the session JWT as a bearer) goes to `getAuth`; an OAuth access token (`typ: at+jwt`) goes to `getMcpTokenAuth`, the same check the download and sync socket accept an agent's token through, so it serves both tldraw.com and a caller holding an OAuth token on a user's behalf. Routing on `typ` means each request is verified once. Refusals are plain: 401, or 403 for a valid token outside `mcp_server_access` — not the MCP endpoint's challenge, whose resource metadata describes that endpoint rather than this one.

**The gate is the point.** The OG route resolves under `access: 'public'`, because it answers strangers and crawlers. This route resolves through `resolveSharedBoardForUser`, the function the MCP server uses to admit a caller to a board: a file the caller can read resolves under `access: 'render'`, and anything else falls back to the published-slug arm under `access: 'public'`. That per-caller resolution is the only reason a private board's thumbnail can be served at all.

A hit is the stored PNG with `cache-control: private, no-cache` and the R2 etag, so a caller revalidates each use for the price of a 304 and every revalidation re-runs the access check. Every miss is a 404 — no such board, not the caller's, nothing drawn yet, or never rendered — so the route is not an existence oracle for file ids, and the caller draws its own placeholder rather than the tldraw-branded default card. A throw from Postgres or R2 is a 500, reported under `board_view`, so an outage is not drawn as a wall of placeholders.

It never asks for a render: that would turn listing boards into a render per tile. Both kinds of miss are counted (`source: board_view`, `failure: board_empty` or `not_rendered_yet`), which is the miss rate that will decide whether to backfill. Images are served at their stored 1200×630; shrinking them to tiles is a follow-up.
...
Bounded reason codes say _that_ a board stopped rendering, never _why_, and every one of these surfaces deliberately swallows its own errors (the OG route falls back to the default image, the snapshot route 404s, the MCP tools return a tool error, the queue retries or drops, and the board view read turns a failed lookup into a 500 and a session lookup that throws into a plain 401, so a Clerk outage looks exactly like a caller who sent no cookie — these reports are what tell those apart). So each swallow point also reports the underlying error to Sentry through `reportThumbnailError` (`thumbnailShared.ts`), tagged `thumbnail_surface` with a closed set of values: `og_route`, `og_queue`, `thumbnail_snapshot`, `mcp_board_info`, `mcp_screenshot`, `mcp_board_search`, `mcp_screenshot_cache_write`, `mcp_cluster_index_read`, `mcp_cluster_index_write`, `board_view`. `mcp_screenshot_cache_write`, `mcp_cluster_index_read`, and `mcp_cluster_index_write` are never caller-visible failures — the tool answered — but each means a cache stopped absorbing what it exists to absorb, so the tools are back to spending Browser Run on every call. Reporting rides on the handler's `waitUntil` and is itself failure-proof — a missing Sentry env var must never turn a degraded-but-fine response into a 500.
...
- There is no kill switch. `POST /app/mcp` is gated by the `mcp_server_access` flag alone, which decides _who_ gets through rather than whether the endpoint answers — and it cannot close it completely, because a verified `@tldraw.com` address is admitted whatever the flag says. Taking the surface down means a deploy that removes the route.
...
The argument for keeping the file-keyed half: an unshared board's thumbnail does depict content that is no longer public, but the image is not public because it exists — it is public because a route serves it, and every route that does re-checks its own gate on every request (`resolveThumbnailBoard` in `getOgImage`; `resolveSharedBoardForUser` in `getBoardThumbnail`, see "The board view read"). An unshared board's image is already unreachable while it sits in R2.

Keeping it means the board view read — an owner-facing surface behind auth — can use the image a board already has, instead of it having been thrown away the moment the board went private. The `og/…` keys carry no version, so retaining them costs one object per board and does not accumulate.
...
- **`access: 'public'`** — every anonymous-facing surface: the OG image route, the crawler HTML (`getSocialPreview`), both MCP tools when the board is a published one, and the board view read's published fallback, which `resolveSharedBoardForUser` resolves under when the per-caller check refuses a board. A published board must be published; a shared file must currently be shared via link (`isFileAnonymouslyViewable`). This is the only thing keeping a private board's thumbnail off the public internet, and it is re-applied per request rather than inferred from what is in R2 — necessarily, since nothing deletes an image when a board stops being public.
- **`access: 'render'`** — the queue consumer, the render page's snapshot route, the board view read, and the MCP tools once `hasReadAccessToFile` has admitted the caller for that file. Requires only that the board exists, is not deleted, is not a test file (`isFileRenderable`), and has persisted content.
...
search_boards({
 ...
 cursor?: string, // the nextCursor from a previous result, replayed with the same query. omit for the first page
})
...
`search_boards` covers the same set the other four will open: boards in the caller's own workspace, boards owned by a workspace they can access files in, and boards shared with them by link that they have opened. That is the scope `hasReadAccessToFile` admits, and the relationship only runs one way — everything search returns must be a board the other tools will open, or a model finds a board and cannot interpret being refused it. A published board stays reachable by id; it is simply not discoverable by name.

**Two reads, ordered by when a board reached the caller.** The access gate has two arms with nothing in common but their answer, so there are two queries, merged in JS through `compareBoardSearchOrder` rather than unioned — that comparator is the single statement of this ordering and the eval harness already pages fixtures with it, so a `UNION`'s own `ORDER BY` would be a third copy to drift. They are issued sequentially because the pool is `max: 1`.

Workspace-owned boards are found by `file."owningGroupId"` — deliberately not through `group_file`, since `hasReadAccessToFile` reads `owningGroupId`, and `group_file` is list membership rather than ownership: `deleteWorkspace` removes those rows by design, and a file can outlive its links (see #10837). A missing link must never be able to hide a board you own. Link-shared boards are found the only way they can be: opening one writes a `group_file` row into the opener's **home** group while the file stays owned elsewhere, which `039_tighten_group_file_association.sql` permits and no other cross-workspace link, and unsharing deletes those rows (`034_fix_unshare_group_file_cleanup.sql`) so the guest set cannot outlive the access that justifies it.

**The sort key is arrival, not creation**, and each read takes it from wherever that lives. For a workspace board it is `file."createdAt"` — not a proxy for arrival but the same value, since `createFile` writes one timestamp into the file row and its `group_file` row alike. For a shared board it is `group_file."createdAt"`, the moment the link was opened. Both are immutable, which is what a keyset cursor needs. The workspace side is index-served by `052_file_search_index.sql`; the shared side is not yet — see below. That ordering is the point — under a creation sort, a board shared with you this morning but made in 2023 lands below everything you have made since. The one case the two diverge is a board _moved_ between workspaces, which keeps its original creation time rather than sorting by the move. Results report `addedAt` alongside `createdAt` so a model can see which is which, and `source` says how the caller reaches each board — `owned`, `workspace`, or `shared`. `shared` is deliberately not a flavour of `workspace`: the caller is not a member of the workspace that owns a link-shared board, so the owning workspace's name is withheld from those rows rather than handed to somebody who was given a link rather than a seat.

**The shared read has no index yet, deliberately.** Both the column it filters on and the one it orders by are on `group_file`, so a single index serves it — there just isn't one, and until there is it fetches every guest link the caller has and sorts them. Measured at production's shape against the heaviest guest-link count (233): 879 buffers, against **89 buffers and 22 rows** with an index on `group_file("groupId", "createdAt" DESC)`. Both are under two milliseconds, and the cost only stops scaling with link count once the index exists.

That index ships as its own deploy, and the reason is a deadlock rather than caution. The runner holds one transaction over every pending migration, so a run building indexes on both `file` and `group_file` holds the first while waiting for the second — and `moveFileToWorkspace` takes those two tables in the opposite order (`group_file` delete, then the `file` update). Reproduced on PG16: `40P01`, with the migration as the victim, so the deploy fails. No lock ordering fixes it, because the app is inconsistent with itself — `createFile` takes `file` then `group_file`. A run that locks only one of the two cannot be in the cycle at all, which is what separate deploys buy. So: `052` ships with the tool, the `group_file` index follows on its own.
...
The seek is written as a row comparison, `(file."createdAt", file.id COLLATE "C") < (:createdAt, :id)`, and that form is load-bearing: Postgres keeps the logically equivalent `createdAt < :c OR (createdAt = :c AND id < :i)` as a filter, so a late page walks every row above the cursor and discards it — page 500 of a 10k-board workspace read 9,901 rows to return 21, against 5 for the row comparison, which PG16 truncates to the leading indexed column and turns into `createdAt <= :c` as an index condition.

The cursor also carries the query that minted it, and a cursor replayed under a different query is refused rather than honoured. Without that, a model that pages and then narrows its query while passing the cursor on continues from row 20 of a set it never saw the start of, with nothing in the result to say so.

A board with no name comes back with `name` empty rather than a stand-in title. tldraw.com titles an unnamed board by its creation date, formatted in the viewer's locale and timezone — neither of which a worker has — so any name invented here would be one the caller cannot see on their own screen, and one no query could match, since the column holds `''`.
...
Search spends no Browser Run, so it sits outside the rate limits below — but unlike `get_board_info`, it is not simply unlimited: it has a budget of its own, because the cost it drives is Postgres rather than a browser session. Paging is now **index-bounded**: `file("owningGroupId", "createdAt" DESC)` (`052_file_search_index.sql`) serves the access filter and the ordering together, so each workspace's page is read in index order and stops — `LIMIT 21` saves work, not just transfer. Nothing indexes `name ILIKE '%…%'`, so a search term is still a filter applied to rows the index scan walks past: cost then scales with how far down the caller's boards the matches sit, and a term matching nothing walks their whole set.

**That part is behind a switch.** `MCP_SEARCH_NAME_MATCHING_ENABLED` gates name matching per environment — `"true"` on staging, `"false"` on production while the scan above is still being watched, and unset reads as on so previews, local dev and tests keep the full tool. Off, `search_boards` becomes a listing tool: it advertises no `query` and refuses one, rather than accepting a query and ignoring it. That distinction is the point — a model that asked for "roadmap" and received the account's twenty most recent boards would read them as twenty matches and open one.

**Why it took two edits, and what still sorts.** The index is usable only because the access predicate is a single equality, which it became when [#10391](https://github.com/tldraw/tldraw/pull/10391) removed the last user-owns-file paths. While the predicate was `file."ownerId" = :caller OR file."owningGroupId" IN (:groups)`, Postgres answered the top-level `OR` with a `BitmapOr`, whose output has no order, so the plan was a bitmap heap scan of everything in scope followed by a top-N sort whatever indexes existed. Measured on Postgres 16.14 against the real DDL and an 8000-board workspace, and re-measured after the merge: with the `OR` arm present the planner materialises ~8010 rows and top-N sorts them, and ignores this index entirely in favour of the existing `file_owning_group_id_idx`; with the arm gone and one workspace, the same query is an index scan plus incremental sort reading 22 rows for a 21-row page. **So don't reintroduce a top-level `OR` in the access predicate** — it silently un-indexes the ordering rather than failing anything. A caller in more than one workspace would produce `owningGroupId = ANY(:groups)`, which is not that single equality — a btree scan driven by an array key is not order-preserving before Postgres 17, and production runs 16 — so that case seq-scanned every board in scope and top-N sorted it: 30,000 rows for the first 21, measured across three 10k-board workspaces. The query therefore asks each workspace for its own page and merges them, `unnest(:groups) CROSS JOIN LATERAL (… LIMIT 21)`, which reads 63 rows for the same answer. The merge is correct because each arm takes a whole page: the global top 21 cannot contain a row missing from every workspace's own top 21. Every filter has to stay inside the arm for the same reason — one applied after the arm's `LIMIT` could empty a page that had more rows to give. The index deliberately stops at `createdAt` and leaves the `file.id COLLATE "C"` tiebreak to the incremental sort: the primary key already makes `id` unique, and ties on `createdAt` are rare.

**Its own limiter, not the Browser Run one.** `MCP_SERVER_SEARCH_RATE_LIMITER` bounds `search_boards` at 60 calls a minute per account (`MCP_SEARCH_PER_USER_RATE_LIMIT`), separately from the per-account Browser Run budget — counting search against that one would make the number `config.ts` documents for it untrue, and would inherit a limit chosen for a different cost.

60 rather than something tighter because **paging is what spends this, not abuse.** The page size is fixed at 20, so a caller in an 8000-board workspace needs 400 calls to walk it, and an agent that reformulates a failed query spends a few more. A refusal mid-page has no good recovery either: a model cannot wait, so it retries or reports a partial answer as a complete one, which is worse than the load it prevents. Against that, the worst measured call is ~9ms of database time, so 60 a minute is well under a second of database time per caller per minute. The refusal is written to the screenshot ledger as `rate_limited_search` even though successful searches are not — that ledger is the panel answering "who is being turned away", and a limit nobody can watch firing is the thing the per-user budget already had to be fixed for once.

What this does **not** bound is many accounts looping at once; that needs a global search limiter alongside it, the way `MCP_SERVER_BROWSER_RATE_LIMITER` sits under the per-user screenshot budget. Deliberately left until there is traffic to size it against, with `postgres_client_connect` volume from this surface as the signal.
...
- **Refusals are on their own event.** `mcp_server_auth_refusal` carries a bounded `reason` (`no_token`, `invalid_token`, `unconfigured`, `not_allowlisted`), the client family, and the `route` that refused it (`mcp`, or `thumbnail` for an OAuth token refused by the board thumbnail route). Nothing else records them — `mcp_server_tool_call` is written by the tools/call dispatcher, which a refused request never reaches — and during a flag-gated rollout "not signed in" and "signed in and not on the list" are the two numbers worth watching, and they call for entirely different responses.
...
flowchart TB
    subgraph entry ["Entry points (sync worker)"]
        ...
        BV["GET /api/app/file/:boardId/thumbnail<br/>(R2 cache read, access-gated, never waits)"]
