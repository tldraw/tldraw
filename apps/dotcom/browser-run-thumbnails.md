...
- `BROWSER` binding - the Cloudflare Browser Rendering binding, declared per environment in `wrangler.toml` (`[env.<env>.browser]`). The worker calls its `quickAction` Quick Actions method (`env.BROWSER.quickAction('screenshot', …)`) directly — no `@cloudflare/puppeteer`, no API token. This requires the worker's top-level `compatibility_date` to be `2026-03-24` or later. The dev binding is marked `remote = true`, so `wrangler dev` runs the worker locally while `BROWSER` proxies to the real remote service; it needs Cloudflare credentials in the local environment, and is left undefined (render path fails closed with a config error) without them.
