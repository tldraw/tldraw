...
**This is not sufficient for a "thumbnails of all boards" surface**, and the gap is worth knowing. A board that has _never_ been shared has never been rendered, so it has no thumbnail at all — `TLFileDurableObject.requestOgRenderForEdit` only proceeds for `shared`/`unknown`, and the consumer's resolve drops anything not public. Nothing deleting images means every board that was _ever_ public still has one, which is not the same as every board having one. Such a surface needs a render path whose gate is ownership rather than public viewability. Both existing gates are there because the render output is served unauthenticated today, so neither can simply be relaxed in place.
