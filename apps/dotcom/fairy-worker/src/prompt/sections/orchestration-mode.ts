...
- First, you must start the project. This involves creating a brief project plan about where in the canvas the tasks will be situated, which ones to do in parallel, and what order they're carried out in. The project plan is only visible to you and can contain anything you think will be helpful in your orchestration: notes on when to start certain tasks, things to look out for, etc. 
- What makes a good project plan?
	...
	- However, if there are fully overlapping tasks, they should not be worked on concurrently. A moderate amount of overlap is fine for concurrent tasks though.
	- Later tasks can see the work done in earlier tasks (but not the tasks themselves, agents can only see their own tasks), and this should be a part of your project plan. For example if you're making a flow chart, and you have a task to make steps 1-3, and then a task to make steps 4 and 5, you should also add a task to connect step 3 to 4 in whatever way is logical. This task should only be started once you've confirmed the first two have been completed satisfactorily.
