export function TldrawLink() {
	return <img className="tldraw-link" src="/tldraw_dev.png" />
}

// export function TldrawLink() {
// 	const ref = useRef<HTMLDivElement>(null)

// 	useLayoutEffect(() => {
// 		if (ref?.current) {
// 			// HACK: Fix for <https://linear.app/tldraw/issue/TLD-1700/dragging-around-with-the-handtool-makes-lots-of-requests-for-icons>
// 			// It seems that passing `WebkitMask` to react will cause a render on each call, no idea why... but this appears to be the fix.
// 			// @ts-ignore
// 			// eslint-disable-next-line deprecation/deprecation
// 			ref.current.style.webkitMask = `url(/tldraw_dev.svg) center 100% / 100% no-repeat`
// 		}
// 	}, [ref])

// 	return (
// 		<div
// 			ref={ref}
// 			className="tldraw-link"
// 			style={{
// 				mask: `url(/tldraw_dev.svg) center 100% / 100% no-repeat`,
// 			}}
// 		/>
// 	)
// }
