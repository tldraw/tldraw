...
-- its own. Stop syncing the name and default it to "Home".
...
-- 2. Default new home groups to "Home" instead of the user's name. Keep the
...
-- 3. Reset existing home group names to "Home". Home workspaces couldn't be
--    renamed before this change, so no existing name is an intentional choice.
UPDATE "group" SET "name" = 'Home' WHERE "id" IN (SELECT "id" FROM public."user");
