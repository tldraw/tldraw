...
-- window on "file" by the build time. No deadlock is possible with it: nothing writes comment_thread
-- and then waits on "file" in one transaction. The comment drain never touches "file" in its
-- transaction, bumpFileUpdatedAt runs on its own connection (TLFileDurableObject), and the
-- comment_thread -> file foreign key check takes a row KEY SHARE, which SHARE doesn't block.
