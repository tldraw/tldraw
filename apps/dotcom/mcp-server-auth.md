...
- Bearer tokens verified with `jose.jwtVerify` against the Clerk instance's JWKS — signature, issuer, `exp` and `typ: at+jwt`. Not `@clerk/backend`'s `verifyToken`, which verifies Clerk _session_ tokens and refuses an OAuth access token on its header alone (`Invalid JWT type "at+jwt". Expected "JWT"`). `@clerk/backend` v3 does accept one through `authenticateRequest` with `acceptsToken: 'oauth_token'`, but that path calls Clerk's Backend API on every request rather than checking the signature locally, so the resource server keeps its own JWKS check ([#10005](https://github.com/tldraw/tldraw/issues/10005)). There is no `aud` check — see the TL;DR and open question 8.
