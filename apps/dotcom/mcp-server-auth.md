...
Client registration is settled: clients identify themselves with Client ID Metadata Documents (CIMD), and we don't support dynamic client registration; a client that lacks CIMD isn't supported until it updates. Token format is settled: access tokens stay JWTs — **but the toggle that decides this is per OAuth application and does not cover CIMD clients**, which were issued opaque `oat_` tokens until it was turned on for them specifically (see [Open questions](#open-questions) 1) — and verification is a local JWKS check through `@clerk/backend` v3's `acceptsToken: 'oauth_token'` path (the v1 SDK's `verifyToken` refused an `at+jwt` on its header alone, which is why this was a `jose` check until [#10005](https://github.com/tldraw/tldraw/issues/10005)). Questions 8–10 were raised by review of the finished branch and are answered too, but 8 is the one to keep in view: with no `aud` to check, the entire client-authorization story is Clerk dashboard state that nothing here can see.
...
- Bearer tokens verified with `@clerk/backend`'s `authenticateRequest` (`acceptsToken: 'oauth_token'`) against the Clerk instance's JWKS — signature, `sub`, `exp` and `typ: at+jwt`. Not `verifyToken`, which is for _session_ tokens and refuses an access token on its header alone (`Invalid JWT type "at+jwt". Expected "JWT"`). Before [#10005](https://github.com/tldraw/tldraw/issues/10005) moved the worker to v3 this was a `jose` check against the authorization server's public JWKS with an explicit `iss` pin; the SDK binds the issuer through the key set instead, since the JWKS it fetches is the accepting instance's own. There is no `aud` check — see the TL;DR and open question 8.
...
**Use the `www` host exactly, for every client.** `tldraw.com/api/app/mcp` answers, but with a Vercel `308` to the `www` form, and a redirect does not change what the client believes the resource is. It then reads metadata advertising `resource: https://www.tldraw.com/api/app/mcp`, finds that is not the URL it was configured with, and refuses before authentication begins — naming the correct URL in the error, which reads like a server fault rather than a typo:

```
Protected resource https://www.tldraw.com/api/app/mcp does not match expected https://tldraw.com/api/app/mcp
```

**Claude, and other clients with a hosted redirect URI.** Add the endpoint as an HTTP MCP server and complete the sign-in the `401` triggers. Claude's CIMD document (`https://claude.ai/oauth/mcp-oauth-client-metadata`) registers `https://claude.ai/api/mcp/auth_callback`, an exact non-loopback URI, so the redirect URI needs no workaround.

The scopes did. Claude requests every scope the authorization server advertises unless the `401` names a narrower set, and Clerk advertises six while granting each CIMD client a subset — so Claude asked for `public_metadata` and `private_metadata`, which its client was not granted, and Clerk refused the whole request with `invalid_scope` before rendering consent. Nothing reached this worker: the only trace was the unauthenticated discovery probe, `401`, and silence. `mcpAuth.ts` now names `MCP_SCOPES` in the challenge and in the RFC 9728 metadata, which is the documented lever for exactly this. Claude Code was never affected — it sends no `scope` at all and gets Clerk's configured defaults — which is what made the failure look client-specific rather than scope-shaped.

**Claude Code needs no workaround any more.** Verified 2026-09-01: Clerk accepts `http://localhost:{port}/callback` off the portless registration, and Claude Code signs in over plain CIMD with no client id. Anthropic's connector documentation now states the port-agnostic match as a requirement on the authorization server for both loopback forms, so this looks like a Clerk change rather than a client one. The rest of this section is kept because it is the fallback if that regresses, and because the two Clerk requirements below fail late and confusingly for anyone who does use an explicit client id.

What it used to do, and what to look for if it comes back: its CIMD document registers the _portless_ `http://localhost/callback` and `http://127.0.0.1/callback`, while the CLI listens on a port and sends `http://localhost:{port}/callback`. Clerk matched redirect URIs exactly, so it refused:
...
1. **Does the Clerk instance issue JWT or opaque access tokens?** JWTs, but not by default everywhere, and the qualifier is the whole answer. **Generate access tokens as JWTs** is a setting on an _OAuth application_, and a CIMD client is not an OAuth application — it has no dashboard record to carry the setting. CIMD clients were therefore issued **opaque** `oat_` tokens while the toggle read as on, and every one of them was refused here: `jwtVerify` cannot parse a 36-character opaque token, so the refusal was `invalid_token` and the client looped through re-authentication getting a fresh, equally unusable token each time. Confirmed 2026-08-28 by token length in Claude Code's MCP log — 36 characters, against 794 for the JWT that replaced it — and fixed by enabling JWTs for the CIMD clients. Verification is a local JWKS check: `@clerk/backend` v3's `acceptsToken: 'oauth_token'` path verifies `at+jwt` against the instance's key set (the v1 SDK's `verifyToken` refused the header type outright, which is why this was a `jose` check before [#10005](https://github.com/tldraw/tldraw/issues/10005)). Opaque `oat_` tokens are the one case the SDK verifies with a Backend API call per request (`idPOAuthAccessToken.verify`), another reason to keep the instance issuing JWTs.
...
8. **What stops a token minted for another MCP server being replayed here, with no `aud` to check?** Clerk's client registry, and nothing in this repo. Any access token the tldraw instance issues — to any client, for any purpose — is full authorization at `POST /api/app/mcp`, so absent that registry a third party could stand up a Clerk OAuth client, walk a tldraw user through consent for its own stated purpose, and replay the resulting token here.
   ...
   Defining one is no longer free, which is worth knowing before it is proposed as the fix. Clerk advertises custom scopes in `scopes_supported`, and a client asks for what it is offered — so the day a custom scope exists, every client requests it, and every client not granted it on the authorization server first is refused with `invalid_scope` at the consent step. That failure surfaces to a user as "check your credentials and permissions", three systems away from the change that caused it. `MCP_SCOPES` bounds the blast radius for clients that read the challenge, but not for a client that enumerates the authorization server's metadata instead.
...
11. **Which scopes should the `401` ask for, and who decides?** `MCP_SCOPES` — `openid profile email offline_access` — named in the `WWW-Authenticate` challenge and in the RFC 9728 metadata. This server reads no scope at all, so the list is not a permission check here; it exists to stop clients guessing, because the guess is expensive. Claude's hosted connectors request everything the authorization server advertises when the challenge is silent, and Clerk advertises `public_metadata` and `private_metadata` alongside the four, granting each CIMD client only a subset — so silence meant `invalid_scope` and a connector that could not be added at all.

    The four are what the CIMD clients are granted, and match what Clerk's defaults already hand a client that sends no `scope`, so both kinds of client now ask for the same thing. Narrower would serve — `sub` is all this server takes from a token, and it rides on `openid` — and narrowing is safe to do here without touching Clerk, since a client may always request less than it was granted.

    The reason this is a question rather than a note: the alternative fix was granting Claude the two metadata scopes, which is what unblocked it before this landed and is **still in place on the instance**. Revoking them is the point of naming scopes here, and it is a dashboard action nothing in this repo can do or check. Until it happens, Claude Desktop tokens can read Clerk private metadata for consenting users — empty today, since nothing in the monorepo reads or writes either metadata field, which is exactly why someone will eventually treat it as free storage.
