...
- Bearer tokens verified with `jose.jwtVerify` against the Clerk instance's JWKS — signature, issuer, `exp` and `typ: at+jwt`. Not `@clerk/backend`'s `verifyToken`, which verifies Clerk _session_ tokens and refuses an OAuth access token on its header alone (`Invalid JWT type "at+jwt". Expected "JWT"`). Since v3, `@clerk/backend` verifies these locally too, through `authenticateRequest` with `acceptsToken: 'oauth_token'`, against a JWKS it fetches from the Backend API with the secret key. The explicit check stays because it pins `iss` to the authorization server the discovery metadata advertises and reads that server's public JWKS, so the two cannot drift ([#10005](https://github.com/tldraw/tldraw/issues/10005)). There is no `aud` check — see the TL;DR and open question 8.
