...
const ourTypeToPostgresType: Record = {
	string: 'text',
	number: 'bigint',
	boolean: 'boolean',
}
function makeColumnStuff(table: string, column: string, details: Omit): ColumnStuff {
	return {
		name: column,
		type: details.type,
		alias: `${table}.${column}`,
...
export function getFetchUserDataSql(userId: string) {
	return sql`
SELECT 'user' AS "table", null::bigint as "mutationNumber", null::text as "lsn", ${sql.raw(
		userColumns + ',' + fileNulls + ',' + fileStateNulls
	)} FROM public.user WHERE "id" = '${sql.raw(userId)}'
UNION
SELECT 'file' AS "table", null::bigint as "mutationNumber", null::text as "lsn", ${sql.raw(
		userNulls + ',' + fileColumns + ',' + fileStateNulls
	)} FROM public.file WHERE ("ownerId" = '${sql.raw(
		userId
	)}' OR "shared" = true AND EXISTS(SELECT 1 FROM public.file_state WHERE "userId" = '${sql.raw(
		userId
	)}' AND public.file_state."fileId" = public.file.id))
UNION
SELECT 'file_state' AS "table", null::bigint as "mutationNumber", null::text as "lsn", ${sql.raw(
		userNulls + ',' + fileNulls + ',' + fileStateColumns
	)} FROM public.file_state WHERE "userId" = '${sql.raw(userId)}'
UNION
SELECT 'lsn' as "table", null::bigint as "mutationNumber", pg_current_wal_lsn()::text as "lsn", ${sql.raw(
		userNulls + ',' + fileNulls + ',' + fileStateNulls
	)}
UNION
SELECT 'user_mutation_number' as "table", "mutationNumber"::bigint as "mutationNumber", null::text as "lsn", ${sql.raw(
		userNulls + ',' + fileNulls + ',' + fileStateNulls
