...
// workspaces as a three-panel comic in the brand's illustration style (the Downy/Cloudy/Penta
// characters), covering shared visibility, inviting people, and moving files in, in place of
// an onboarding modal; the file itself is an ordinary file that the user can rename, edit, or
// delete.
//
// The template is stored pre-serialized as the JSON of a RoomSnapshot: the worker writes the
// string to R2 verbatim and parses a fresh snapshot per seed, so worker isolates don't pay
// object-graph evaluation at cold start and no mutable snapshot object is shared between
// rooms (templates.test.ts checks the JSON parses into the expected shape).
//
// To regenerate after redesigning the canvas: open a file in the app, build the content,
// then run this in the console and rebuild the RoomSnapshot JSON between the backticks
// below ({ documentClock: 0, tombstoneHistoryStartsAtClock: 0, schema, documents:
// records.map((state) => ({ state, lastChangedClock: 0 })) }):
//
//   JSON.stringify({
...
export const newWorkspaceTemplateJson = `{
	"documentClock": 0,
