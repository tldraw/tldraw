...
3. `yarn prune:run --dry-run` → idle histogram, no writes. Results append to `prune-dry-run.jsonl` and progress is checkpointed to `prune-progress.json` as a line offset into `prune-ids.txt`, so a killed run resumes and re-does at most a few batches. The offset only applies to a matching pass: changing `--max-idle` or dropping `--dry-run` starts from line 0, because ids kept by a shorter threshold must be re-evaluated.
