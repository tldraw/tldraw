...
5. Rotate `ADMIN_TOKEN` (`npx wrangler secret put ADMIN_TOKEN` with a new value, or delete it) after the prune; the route stays but it should not keep a live token between runs.
