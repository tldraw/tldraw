...
completeness: 8
accuracy: 8
notes: 'Strong voice with good code examples. Great coverage of custom shapes, meta, rich text. Could expand on ShapeUtil flags and interaction sections.'
---
...
Custom shapes can clip their children so content outside the parent's boundary is hidden. Implement the `getClipPath` method to define the clipping polygon, and optionally `shouldClipChild` to control which children are clipped. See [Shape clipping](/sdk-features/shape-clipping) for the full API and examples.
