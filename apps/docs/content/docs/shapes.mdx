...
class CardShapeUtil extends ShapeUtil<CardShape> {
  ...
  onResize: TLOnResizeHandler<any> = (shape, info) => {
  	const { newPoint, scaleX, scaleY } = info

    let w = Math.max(1, shape.props.w * scaleX)
    let h = Math.max(1, shape.props.h * scaleY)
...
> It's important that this class have a static type property that matches the type of your shape.