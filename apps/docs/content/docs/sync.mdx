...
tldraw sync supports loading [`TLStoreSnapshot`](?) snapshots via the [`InMemorySyncStorage`](?) class, so in your data loading code you can add a backwards-compatibility layer that lazily imports data from your old system and converts it to a `TLStoreSnapshot`.

Something like

```tsx
import { DEFAULT_INITIAL_SNAPSHOT, InMemorySyncStorage, TLSocketRoom } from '@tldraw/sync-core'
