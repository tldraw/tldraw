...
While we offer a [hosted demo](/collaboration#tldraw-sync-demo) of the tldraw sync library for prototyping, the demo is suitable only for prototyping. To use tldraw sync in production, you will need to self-host the tldraw sync server.

For other collaboration options, see our [collaboration](/collaboration) page.

## Deploying the tldraw sync backend
...
Use [`createTLSchema`](?) to create a store schema, and pass that into [`TLSocketRoom`](?). You can use shape/binding utils here, but schema will only look at two properties: [`props`](/reference/editor/ShapeUtil#props) and [`migrations`](/docs/persistence#Shape-props-migrations). You need to provide the default shape schemas if you're using them.
