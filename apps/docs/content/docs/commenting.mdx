...
Commenting is a licensed feature. It runs in development without a key. In production it needs a tldraw license that includes commenting; without one the commenting UI disappears — the toolbar button, its shortcuts, and the layer itself. The rest of the editor is unaffected.
...
`canComment` decides whether the viewer may participate, and the UI follows it: composers give way to a fallback slot and the action affordances hide.

It's a UI-level control and nothing more. Comment records carry a client-supplied author id, so rules about who may post, edit, or delete belong in your sync server's record authorization, where each incoming record is checked against the session's identity.
