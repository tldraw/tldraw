...
The tldraw SDK includes deep support for real-time collaboration. The easiest way to add collaboration to your project is through our [tldraw sync](#tldraw-sync) library. If you prefer to use something else, you can integrate [other backends](#Using-other-collaboration-backends) using our other public APIs.
...
[tldraw sync](/sync) is our library for fast multiuser collaboration. It's purpose-built for the tldraw canvas and it's what we use to power collaboration on our flagship app tldraw.com.
...
Whichever backend you choose, adding collaboration to tldraw will involve sychronizing data, sharing user presence, and handling user assets.
...
You can use the [Editor](?)'s asynchronous external asset handlers to generate assets from content that a user drops, pastes, or adds to the canvas. For example, you might upload images to a storage bucket and return an asset that includes their source URL; or pull favicons and social images in order to create bookmarks from pasted URLs.
...
Read more about [how assets work in tldraw](/docs/assets). You can also refer to our simple example [client](https://github.com/tldraw/tldraw/blob/main/apps/simple-server-example/src/client/App.tsx) and [server](https://github.com/tldraw/tldraw/blob/main/apps/simple-server-example/src/server/unfurl.ts) code for examples of asset handling.
