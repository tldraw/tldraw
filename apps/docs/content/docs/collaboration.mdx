...
Tldraw ships with a plug-and-play sync engine with multiplayer support.

### Using the demo server

You can easily get started prototyping a multiplayer experience by using our free hosted demo server (data expires after one day).
...
You can connect to or create a tldraw board by passing a unique `roomId` to the `useMultiplayerDemo` hook (all data is public so use a randomized id to avoid collisions).

View the [live example](/examples/basic/multiplayer-demo) to see it in action.

### Self-hosting the tldraw sync backend
