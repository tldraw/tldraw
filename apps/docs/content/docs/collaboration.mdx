...
While **tldraw sync** is purpose-built for tldraw and remains the **recommended** approach for adding real-time collaboration, you can also integrate it with third-party backends if your project requires it.

For example, here are two implementations using [Liveblocks](https://liveblocks.io/multiplayer-editing): [Liveblocks + Storage](https://liveblocks.io/examples/tldraw-whiteboard) and [Liveblocks + Yjs](https://liveblocks.io/examples/tldraw-whiteboard/nextjs-tldraw-whiteboard-yjs).
