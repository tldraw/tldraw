...
It's possible to export the contents of the canvas to an image. The easiest way to do this is with [`exportToBlob`](?).

Image exports work by first converting the contents of the canvas to an SVG, then converting that SVG to another format (like a PNG) if needed. Use [`Editor#getSvgElement`](?) or [`Editor#getSvgString`] if you want to work with the generated SVG directly.

### Exporting custom shapes

By default, custom shape exports use the [`ShapeUtil#component`](?) method and embed the resulting HTML in an SVG `<foreignObject>` element. This works well for most shapes, but isn't perfect (see below). For more control over custom shapes, you can implement the [`ShapeUtil#toSvg`](?) method to properly convert your shape to an SVG - this works especially well if your shape is already an SVG.

Use the [`useSvgExportContext`](?) hook to check whether your shape is currently being rendered for an SVG export. If you need to load data or assets asynchronously, you can use the [`useDelaySvgExport`](?) hook to delay the export until you're ready.

SVGs can't load any external assets like images, fonts, or CSS. Everything needs to be included inline in the SVG, with images and fonts converted to data URLs. `<foreignObject>` exports that use the `component` methods try to handle this automatically, but if you implement `toSvg` you'll need to do it yourself. You can use [`SvgExportContext#addExportDef`](?) to add definitions to the SVG element that can be shared across multiple elements.

### Export limitations

`toSvg` based exports can make full use of the SVG format, and typically are very reliable and have good compatibility with other SVG tools outside the browser. `component` based exports that rely on `<foreignObject>` aren't as reliable, as many SVG editors (and even some viewers) don't support them at all. They work well in the browser though, and if you export to a raster format like PNG you won't need to worry about compatibility other than in the browser creating the image.

Even within browsers though, `<foreignObject>`s and the method we use to create them aren't always 100% reliable. Chromium based browsers tend to work well, but Firefox occasionally has layout issues, and Safari has rendering quirks with effects like box shadows. It's sometimes useful to work around these by detecting particular browser & export combinations, and switching styling accordingly.

We can only embed external assets such as images and font files if they're either hosted on the same origin as tldraw, or have [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) headers set. If external assets are missing from your exported SVG, check your browser's network tab to see if these are being requested and if anything is going wrong with them.
