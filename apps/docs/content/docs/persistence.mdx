...
completeness: 8
accuracy: 8
notes: 'Good progressive disclosure from simple to complex. Code examples are complete. Minor: await in non-async onClick handlers (lines 78-79, 96-99).'
---
	...
	sequence: [
		{
			id: Versions.AddColor,
			// Scope can be one of
			// - 'storage' to have the up function receive a storage object with get/set/delete methods for the entire document
