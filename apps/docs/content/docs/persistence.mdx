...
If you have a custom shape type, you can define a `migrations` property on the shape util class. Use the `createShapePropsMigrations` helper to define this property.

```ts
import { createShapePropsMigrations, createShapePropsMigrationIds, ShapeUtil } from 'tldraw'
...
class MyCustomShapeUtil extends ShapeUtil {
	...
	static migrations = createShapePropsMigrations({
		sequence: [
			{
...
```ts
import { createMigrations, isShape } from 'tldraw'

const myMigrations = createMigrations({
	sequenceId: SEQUENCE_ID,
	// Setting 'retroactive' to true will make sure the migration runs when loading snapshots
	// that were created before this sequence existed. You almost certainly want this.
	// Setting 'retroactive' to false is mainly useful when publishing a 3rd party tldraw plugin to npm.
	retroactive: true,
	sequence: [
		{
			id: Versions.AddColor,
...
And finally pass your migrations in to tldraw. Where you do this will depend on how custom your setup is, but it's probably one or two of these:
...
2. Replace your `defineMigrations` call with `createShapePropsMigrations`
...
```ts
const migrations = createShapePropsMigrations({
	sequence: [
		{
			id: Versions.AddColor,
