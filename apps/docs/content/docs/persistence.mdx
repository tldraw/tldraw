...
You can get a JSON snapshot of the document content and UI 'session' state using the [getSnapshot](?) function.
	...
	return (
		<button
			onClick={() => {
				...
				// If you are building a multi-user app, you probably want to store the document and session separately
				// because the session is user-specific.
...
You can load the snapshot into a new editor with the `snapshot` prop.
...
You can load the snapshot into an existing editor with [loadSnapshot](?).
...
Note that loading a snapshot does not reset the editor's tool state. For example, loading a snapshot during a resizing operation may lead to a crash. This is because the resizing state maintains its own cache of information about which shapes it is resizing, and its possible that those shapes may no longer exist! If you need to reset the editor's tool state, you can call `editor.complete()` before loading the snapshot.
