...
You can get a JSON snapshot of the document content and UI 'session' state using the [Editor#getSnapshot](?) method.
...
You can load the snapshot into an existing editor with [Editor#loadSnapshot](?).
