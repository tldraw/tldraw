...
You can also pass a snapshot as a prop to set the initial editor content.
...
Note that loading a snapshot does not reset the editor's tool state. For example, loading a snapshot during a resizing operation may lead to a crash. This is because the resizing state maintains its own cache of information about which shapes it is resizing, and its possible that those shapes may no longer exist after the snapshot loads!

To avoid this, you can call `editor.setCurrentTool('select')` to reset the tool state before loading a snapshot.
