...
This will use the custom icon for the `hand` tool and the default icons for all other tools.
...
```javascript
import { getAssetUrlsByMetaUrl } from '@tldraw/assets/selfManaged'
...
You also need to setup your bundler to use these assets. For webpack you can do this with [Asset Modules](https://webpack.js.org/guides/asset-modules/). An example config might look like this:
```javascript
module: {
    rules: [
        {
