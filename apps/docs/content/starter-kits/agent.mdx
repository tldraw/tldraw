...
The Agent Starter Kit demonstrates how to build an AI agent that can interpret and manipulate the [t](https://github.com/tldraw/tldraw)ldraw canvas. It features a chat panel on the right-hand-side of the screen where the user can communicate with the agent, add context and see chat history. It works as a foundation to create diagram generators, drawing assistants, or visual AI applications.

[See Demo](https://agent.tldraw.com)
...
To make decisions on what to do, the agent gathers the information from various sources:
...
**Custom shape integration**: Learn how to enable your agent to understand and manipulate custom shape types specific to your domain or application requirements.

**Real-time collaboration**: Learn how to build multi-user agent environments where multiple AI assistants and human users can collaborate on the same canvas simultaneously.
