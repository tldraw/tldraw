...
embed: multiplayer
githubLink: https://github.com/tldraw/tldraw/tree/main/templates/sync-cloudflare
keywords:
  - multiplayer
  - sync
  - collaboration
...
The Durable Object uses lazy room initialization with WebSocket hibernation support. When all clients disconnect, the DO hibernates to save memory while keeping WebSocket connections alive at the Cloudflare layer. When clients reconnect, sessions resume from snapshots.

See `worker/TldrawDurableObject.ts` for the full implementation:
