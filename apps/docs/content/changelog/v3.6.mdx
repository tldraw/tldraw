...
Actions and toasts overrides now have access to a wider range of helper functions, including clipboard interactions, toast notifications, export utilities, printing, and more. This makes it easier to create custom actions and tools that integrate deeply with tldraw's functionality. ([#5041](https://github.com/tldraw/tldraw/pull/5041))
...
- Removed `ExtractOptionalKeys` and `ExtractRequiredKeys` utility types. Use the new utility type for creating optional properties instead. ([#5055](https://github.com/tldraw/tldraw/pull/5055))
...
- Notes now properly snap to the grid after their position is updated, improving alignment consistency ([#5010](https://github.com/tldraw/tldraw/pull/5010))
