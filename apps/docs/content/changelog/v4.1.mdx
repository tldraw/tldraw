...
- Reinstate focus mode action. ([#6812](https://github.com/tldraw/tldraw/pull/6812))
...
Move `InFrontOfTheCanvas` component back out of the `.tl-canvas` element. The component was nested inside `.tl-canvas` in v4.0 for sizing constraints, but this broke some subtle interactions involving menus and scrolling. If you need to match custom styling on the `.tl-canvas` element, apply identical styling to `.tl-canvas__in-front`. ([#7021](https://github.com/tldraw/tldraw/pull/7021))
