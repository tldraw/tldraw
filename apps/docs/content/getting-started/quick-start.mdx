...
collaborating in realtime, you can user our multiplayer demo server from `@tldraw/sync`:

```bash
npm install @tldraw/sync
```

Then, set up your component with our sync demo server:

- Import the `useMultiplayerDemo` hook from the `@tldraw/sync` package
