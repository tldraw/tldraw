...
[View on GitHub](https://github.com/tldraw/tldraw/releases/tag/v3.10.0)
...
- With the new `richText` functionality, the `text` prop on most shapes has been removed and replaced by a new `richText` prop.
  - When setting this prop, replace `text: 'some string'` with `richText: toRichText('some string')`.
  - When getting this prop, replace `props.text` with `renderPlaintextFromRichText(props.text)`.
