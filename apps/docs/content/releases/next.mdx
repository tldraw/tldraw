...
### Right-click pan ([#8501](https://github.com/tldraw/tldraw/pull/8501))

You can now pan the camera by right-click-dragging on the canvas. A static right-click still opens the context menu on release. Set the new `rightClickPanning` editor option to `false` to restore the previous behavior where right-click opens the context menu on press without panning.

### Performance measurement API ([#8421](https://github.com/tldraw/tldraw/pull/8421))

A new `editor.performance` manager lets you observe frame times, long animation frames, and interaction latency in your tldraw app. Subscribe with `editor.performance.on('event', handler)` to receive flat, analytics-friendly events with percentile stats ready to forward to services like Datadog or PostHog.

```tsx
const unsubscribe = editor.performance.on('interaction-end', (event) => {
	console.log(event.avgFrameTime, event.p95FrameTime)
})
```

The manager is zero-overhead when no listeners are attached and automatically captures Long Animation Frame (LoAF) data during interactions and camera operations.
...
- Add `TldrawOptions.rightClickPanning` (default `true`) to gate right-click + drag camera panning. Add `InputsManager.getIsRightPointing()` and `InputsManager.setIsRightPointing()`. ([#8501](https://github.com/tldraw/tldraw/pull/8501))
- Add `editor.performance` with `PerformanceManager` for observing frame times, long animation frames, and interaction latency. ([#8421](https://github.com/tldraw/tldraw/pull/8421))
- Add `'none'` value to `DefaultDashStyle` / `TLDefaultDashStyle` to hide shape borders while preserving fill. Add `NonePathBuilderOpts` to `PathBuilder`; `PathBuilder.toSvg()` returns `null` and `PathBuilder.toPath2D()` returns an empty `Path2D` for the `'none'` style. ([#8453](https://github.com/tldraw/tldraw/pull/8453))
...
- 💥 Change the default `Cmd+Shift+C` / `Ctrl+Shift+C` "copy as" keyboard shortcut from SVG to PNG. ([#8532](https://github.com/tldraw/tldraw/pull/8532))
...
- Fix `ShapeUtil.canBind` and `ShapeUtil.canReceiveNewChildrenOfType` signatures so overrides can declare the `shape` parameter without TypeScript errors. ([#8521](https://github.com/tldraw/tldraw/pull/8521))
...
- Fix crash in browsers without `OffscreenCanvas` support (e.g. older Safari). ([#8582](https://github.com/tldraw/tldraw/pull/8582))
