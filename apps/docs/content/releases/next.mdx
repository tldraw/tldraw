...
- 💥 Add a public `EditorManager` base class so editor-attached managers share one teardown contract. `InputsManager`, `TickManager`, `FocusManager`, and `TextManager` now extend it. As part of this, `TextManager`'s `editor` field is now protected and its `dispose()` method is internal. ([#9204](https://github.com/tldraw/tldraw/pull/9204))
