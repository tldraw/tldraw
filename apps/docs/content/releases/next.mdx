...
This release adds SQLite storage support for sync, new configuration options for zoom, snap threshold, and frame resizing, along with significant improvements to shape and binding type safety through module augmentation. It also includes React 19 support, various performance optimizations, and numerous bug fixes.
...
### Augmentation-based shape and binding types ([#7091](https://github.com/tldraw/tldraw/pull/7091))
...
- **Breaking:** `TLDrawShapeSegment.points` type changed from `VecModel[]` to `string` (base64-encoded). Added `scaleX` and `scaleY` properties to draw and highlight shapes. Added `b64Vecs` encoding utilities and `getPointsFromDrawSegment` helper functions. ([#7364](https://github.com/tldraw/tldraw/pull/7364))
