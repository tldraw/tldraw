...
- Add `flipX` and `flipY` props to `TLGeoShapeProps`, with an `AddFlipProps` migration that defaults both to `false`. Also adds `PathBuilder.transform(mat)`, which returns a transformed copy of a path. ([#9530](https://github.com/tldraw/tldraw/pull/9530))
...
- Add a `sizeToContentAspectRatio` flag to `EmbedDefinition` and an `EmbedShapeUtil.resolveAspectRatio(shape)` method. When the flag is set, a new embed resolves its content's real aspect ratio from the URL's OpenGraph metadata and corrects its size. Enabled for the built-in Vimeo definition. ([#9613](https://github.com/tldraw/tldraw/pull/9613))
- Extract the "Export as" context submenu into an `ExportAsMenuGroup` component, mirroring `CopyAsMenuGroup`. `CopyAsMenuGroup`, `ExportAsMenuGroup`, and `ExportFileContentSubMenu` now return `null` when their actions have been removed through `overrides`, so hiding the actions hides the submenu instead of leaving an empty trigger. ([#9589](https://github.com/tldraw/tldraw/pull/9589))
- Populate `sources` on external content for file pastes. A `files` handler or `onBeforePasteFromClipboard` can now read the text, html, and url content that was on the clipboard alongside a pasted file, which previously got discarded. ([#9763](https://github.com/tldraw/tldraw/pull/9763))
...
- Add an `EditorManager` base class for editor-attached managers that own a subscription or a resource. A subclass registers its teardown next to the setup that needs it, and the base class runs that teardown on `dispose()`. `InputsManager`, `TickManager`, `FocusManager`, and `TextManager` now extend it. ([#9204](https://github.com/tldraw/tldraw/pull/9204))
...
- Fix a crash when dragging content from another browser tab whose URL isn't a valid link, which happens when an ad blocker rewrites the dragged URL. Dropping one now shows an error toast instead of failing validation on the bookmark shape. ([#9598](https://github.com/tldraw/tldraw/pull/9598))
