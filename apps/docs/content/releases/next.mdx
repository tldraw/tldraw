...
- 💥 Type the collaborator user-id APIs as `TLUserId` instead of `string`. `TLInstancePresence.userId` / `followingUserId`, `TLInstance.followingUserId` / `highlightedUserIds`, `Editor.startFollowingUser()`, `Editor.zoomToUser()`, `usePresence()`, `usePeerIds()`, and the people-menu prop types now use `TLUserId`. Runtime behavior is unchanged. ([#9002](https://github.com/tldraw/tldraw/pull/9002))

  <details>
  <summary>Migration guide</summary>

  `TLUserId` is a branded string. If you store or pass user ids around, cast plain strings at the boundary:

  ```ts
  import { TLUserId } from 'tldraw'
