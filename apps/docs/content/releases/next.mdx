...
Embed definitions are now configured through `EmbedShapeUtil.configure()` instead of the static `setEmbedDefinitions()` method, which has been removed. The `embeds` prop on `Tldraw` is deprecated in favor of this approach.
...
- 💥 **`EmbedShapeUtil.setEmbedDefinitions()`** removed. Use `EmbedShapeUtil.configure({ embedDefinitions: [...] })` instead. ([#8034](https://github.com/tldraw/tldraw/pull/8034))
