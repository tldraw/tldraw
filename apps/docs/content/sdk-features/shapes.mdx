...
completeness: 8
accuracy: 6
notes: "Found several API inaccuracies: focusGroup should be setFocusedGroup, getShapeOpacity doesn't exist, onResize return type is partial not full shape, hitTestPoint is not on ShapeUtil, hideInCulledBounds/isShapeCulled don't exist."
---
...
The `onResize` hook requires careful implementation. When a shape resizes, the hook receives resize information including the scale factors and the handle being dragged, and returns a partial shape update with the changed properties:
...
Shape opacity multiplies with parent opacity. A shape at 50% opacity inside a frame at 50% opacity renders at 25% opacity. The editor tracks this computed opacity for rendering. Access a shape's opacity directly from the shape record via `shape.opacity`.
...
Tracks which shapes are outside the viewport. Shapes whose ShapeUtil returns `true` from `canCull()` are candidates for culling. Check if a shape is currently culled with `getCulledShapes().has(shapeId)`.
