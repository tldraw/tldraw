...
Here's a pattern for auto-saving with throttling, using the `throttle` helper that tldraw exports:

```ts
import { getSnapshot, throttle } from 'tldraw'
...
- [Meta migrations](/examples/data/assets/meta-migrations) - General migrations for meta properties
