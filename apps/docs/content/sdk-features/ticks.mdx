...
completeness: 9
accuracy: 9
notes: 'Well-structured with accurate code. Minor: bolded list in internal section could be prose per VOICE.md.'
---

The tick system provides a frame-synchronized update loop for the editor. The editor emits `tick` events on every animation frame using `requestAnimationFrame`, passing the elapsed time since the last frame. This enables smooth animations, edge scrolling during drag operations, and time-based state updates.
