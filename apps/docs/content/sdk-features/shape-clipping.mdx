...
completeness: 7
accuracy: 9
notes: "Fixed critical error in hit testing section that incorrectly claimed clipping doesn't affect selection. Also removed reference to nonexistent ShapeUtil.hitTestPoint method. All sections now accurate."
---

Shape clipping lets parent shapes mask their children so content outside the parent's boundary is hidden. Frames use this to contain their content—drop shapes into a frame and they're cropped to the frame's edges. Custom shapes can define their own clip boundaries using any polygon, creating circular masks, star-shaped windows, or whatever geometry your application needs.
...
```typescript
import { ShapeUtil, Vec } from 'tldraw'
...
For shapes that should accept children, also override `canReceiveNewChildrenOfType`:
...
This lets you create clipping shapes where some content types break out of the boundary. You might clip geometric shapes but let labels extend beyond the edge, or clip everything except highlighted elements.
...
The editor computes masks for any clipped shape. Get the computed mask with `getShapeMask`:

```typescript
const mask = editor.getShapeMask(shapeId)
// Returns array of Vec points in page space, or undefined if not clipped
```

For the CSS clip-path string, use `getShapeClipPath`:
...
When true, shapes with `backgroundComponent` methods have their backgrounds rendered above this shape rather than above the canvas background. This creates proper visual layering within clipping containers.

## Clipping and hit testing

Clipping affects both rendering and hit testing. The Editor's `getShapeAtPoint` method filters out shapes when the click point falls outside the shape's mask—you can't select a clipped shape by clicking its hidden portions.

Snapping and bounds calculations use the shape's full geometry, so a clipped shape's bounds may extend beyond what's visible. Only the visual representation and hit testing are masked.

## Related examples

- **[Custom clipping shape](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/custom-clipping-shape)** - Create circular clipping shapes with toggleable clipping and selective child clipping.
