...
Clipping affects rendering, not hit testing. A clipped shape can still be selected by clicking its hidden portions. The shape's full geometry is used for selection, snapping, and bounds calculation—only the visual representation is masked.

If you need hit testing to respect clip boundaries, implement custom logic in your ShapeUtil's `hitTestPoint` method by checking if the point falls within the ancestor's clip path.
