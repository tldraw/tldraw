...
By default, pasting a URL from a supported service creates an embed shape. Set `embedOnPaste: false` in the embed definition to create a bookmark instead. Pasting over a shape that can hold a link sets that shape's link rather than creating either.
