...
completeness: 9
accuracy: 9
notes: 'Improved voice with direct statements, removed trailing gerunds. Added API cross-links throughout. Tables for mode/state information. Reactive UndoButton example. Corrected canUndo/canRedo as reactive properties. Better example links (store-events instead of canvas-events).'
---
...
[Editor#bail](?) returns to the most recent mark. [Editor#bailToMark](?) returns to a specific mark by ID.
