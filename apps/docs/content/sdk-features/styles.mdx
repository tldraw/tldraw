...
completeness: 9
accuracy: 9
notes: "Well-structured coverage of styles system. Clear progression from basics to custom styles. All API methods verified correct. Code snippets syntactically valid. Good use of getAsKnownValue helper. Examples all exist and properly linked."
---
...
The editor computes shared styles across the current selection. Use `getSharedStyles` to get a map of each style property to its status: either "shared" (all shapes have the same value) or "mixed" (shapes have different values).
...
You can set the style for next shapes without affecting the current selection:
