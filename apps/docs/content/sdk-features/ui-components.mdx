...
completeness: 7
accuracy: 7
notes: "Good structure with clear sections. CustomToolbar example has incorrect useTools() usage (returns object not array). Example links use non-existent category paths. Voice is confident with minimal hedging. Could use more links to actual examples."
---
...
- **[Custom UI](/examples/custom-ui)** — Build a completely custom toolbar UI while using the editor's functionality
- **[Custom components](/examples/custom-components)** — Override individual canvas components like the brush selection box and scribble
- **[Hide UI](/examples/hide-ui)** — Use hideUi to hide the entire default interface
- **[UI components hidden](/examples/ui-components-hidden)** — Selectively hide specific UI components
- **[Custom menus](/examples/custom-menus)** — Create custom menus using tldraw's menu system
- **[Vertical toolbar](/examples/vertical-toolbar)** — Rearrange the toolbar layout
