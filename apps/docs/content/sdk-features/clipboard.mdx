...
When you copy or cut shapes, the editor calls [getContentFromCurrentPage](?) to serialize them into a [TLContent](?) object:
...
[putContentOntoCurrentPage](?) handles paste operations. It takes [TLContent](?) and reconstructs shapes on the current page:
...
Before writing to the clipboard, the editor calls [resolveAssetsInContent](?) to convert asset references into data URLs:
...
[putContentOntoCurrentPage](?) offers flexible positioning:
