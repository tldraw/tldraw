...
A collaborator whose cursor is outside your viewport shows as a small hint arrow clamped to the viewport edge, pointing toward them.

To customize the rendering, pass your own components via the `components` prop. The `CollaboratorCursor` slot replaces the cursor (see [DefaultCursor](?) and [TLCursorProps](?)); the `CollaboratorHint` slot replaces the off-screen hint (see [DefaultCollaboratorHint](?) and [TLCollaboratorHintProps](?)):

```tsx
import { TLCursorProps, Tldraw } from 'tldraw'
import 'tldraw/tldraw.css'

function CustomCollaboratorCursor({ point, color, name }: TLCursorProps) {
