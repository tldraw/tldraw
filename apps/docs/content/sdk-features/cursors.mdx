...
Cursors were previously drawn to the overlay canvas by [CollaboratorCursorOverlayUtil](?). That util is deprecated but still works: register it (or a subclass) through the `overlayUtils` prop and the DOM cursor layer stands down, so cursors are never drawn twice.

```tsx
import { CollaboratorCursorOverlayUtil, defaultOverlayUtils, Tldraw } from 'tldraw'

export default function App() {
	// Draws cursors to the canvas, as before
	return <Tldraw overlayUtils={[...defaultOverlayUtils, CollaboratorCursorOverlayUtil]} />
}
```
