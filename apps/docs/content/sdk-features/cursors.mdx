...
Remote cursors render through [CollaboratorCursorOverlayUtil](?), one of the default [overlay utils](/sdk-features/overlay-utils). It draws each collaborator's cursor directly to the canvas:
...
The render context is transformed to page space, so divide screen-pixel sizes by the zoom level to keep them constant on screen. See [Overlay utils](/sdk-features/overlay-utils) for the full overlay API.
...
- [Sticker (shape, tool, and bindings)](/examples/shapes/tools/sticker) - Build a custom tool with cursor handling
