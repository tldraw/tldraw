...
The ancestor chain is the path from a shape up to the page. Use `editor.getShapeAncestors()` to get all ancestors in order from the immediate parent to the root:

```typescript
// For a deeply nested shape:
// page > frameA > groupB > myShape
const ancestors = editor.getShapeAncestors(myShapeId)
// Returns: [groupB, frameA]
```

The array is ordered from immediate parent to root. The page itself is never included—ancestors only contain shapes.
...
const commonFrame = editor.findCommonAncestor(
	shapeIds,
...
To collect all descendants including the shape itself:
...
To get the page a shape belongs to:
...
Similarly, shapes are hidden if any ancestor is hidden. The editor tracks this through `editor.isShapeHidden()`:
