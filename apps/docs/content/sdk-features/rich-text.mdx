...
Code is the exception to both. Typing inside inline code, or between a backtick and the closing one that turns the text into code, leaves punctuation as typed, so `` `say "hi"` `` keeps its straight quotes. And a backtick over a selection toggles the code mark on it instead of wrapping it in literal backticks.
