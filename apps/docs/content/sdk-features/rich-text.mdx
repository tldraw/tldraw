...
Typing `[ ] ` or `[x] ` at the start of a line turns it into a task list, as does the checklist button in the rich text toolbar (cmd/ctrl+shift+9). Tab and Shift+Tab nest and un-nest items.

Checkboxes are only clickable while the shape is being edited. Outside of edit mode the text is static HTML, so a click there would tick the box without ever reaching the shape.
