...
[tipTapDefaultExtensions](?) is TipTap's StarterKit plus tldraw's customizations: the `Highlight` mark, `TaskList` and `TaskItem` for checkbox lists, TipTap's `Typography` rules, [WrapSelectionExtension](?), a Shift+Enter tweak, and automatic text direction for right-to-left languages. StarterKit is configured to disable blockquotes, code blocks, and horizontal rules; headings, lists, links, and the standard marks stay on. Links don't open on click during editing, which prevents accidental navigation.
...
Typing `[ ] ` or `[x] ` at the start of a line turns it into a task list, as do the markdown spellings `[] `, `-[ ] ` and `- [ ] ` (or `x` in place of the space), and the checklist button in the rich text toolbar (cmd/ctrl+shift+9). Tab and Shift+Tab nest and un-nest items.

With the select tool, clicking a checkbox ticks it in one click, whether or not the shape is being edited. Outside of edit mode the click toggles the item on the shape directly and doesn't select the shape or start a drag; hold Shift, Alt or cmd/ctrl to select it instead.

`TaskList` and `TaskItem` aren't part of StarterKit, so unlike headings or lists you turn them off by filtering them out of the extension list rather than through StarterKit options:

```tsx
import { tipTapDefaultExtensions } from 'tldraw'

const extensions = tipTapDefaultExtensions.filter(
	(extension) => extension.name !== 'taskList' && extension.name !== 'taskItem'
)
```

### Typography

Typography rewrites punctuation as it's typed: quotes curl, `--` becomes an em dash, `...` an ellipsis, `->` an arrow. Backspace immediately after a rewrite restores what was typed. `WrapSelectionExtension` handles the other half: with text selected, typing either half of a bracket or quote pair wraps the selection rather than replacing it, so selecting `hello` and pressing `¡` — or the `!` that closes it, which is on every keyboard — gives `¡hello!`. See [defaultWrappingPairs](?) for the pairs, which the extension's `pairs` option replaces.

Code is the exception to both. Typing inside inline code, or between a backtick and the closing one that turns the text into code, leaves punctuation as typed, so `` `say "hi"` `` keeps its straight quotes. Wrapping code in quotes uses straight ones too, and a backtick over a selection toggles the code mark on it instead of wrapping it in literal backticks. Text in a monospace font, such as a shape with the mono font, keeps straight quotes as well, both when typed and when wrapping; the other rewrites still apply there.

To opt out of either, filter it from the list — both are separate entries in the array, unlike the StarterKit sub-extensions:

```typescript
import { getTipTapDefaultExtensions } from 'tldraw'

const extensions = getTipTapDefaultExtensions().filter((e) => e.name !== 'typography')
```
