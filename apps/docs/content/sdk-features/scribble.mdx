...
| Stopping | The scribble fades out by progressively removing points from its tail. Once all points clear, the manager deletes it. |
...
When `selfConsume` is `false`, points accumulate while the session is active and only fade after the session stops. The laser pointer uses this with grouped fade mode so all strokes from a drawing session disappear together.

## Customizing scribble rendering

You can override the scribble component to customize how scribbles render. Pass a custom `Scribble` component through [TLEditorComponents](?). The component receives [TLScribbleProps](?) with `scribble`, `zoom`, `color`, `opacity`, `className`, and optionally `userId` for collaborator scribbles.

To customize how other users' scribbles appear in multiplayer, override the `CollaboratorScribble` component instead.
...
The default `Scribble` component in tldraw uses the [getStroke](?) function from the freehand library to create smooth, pressure-sensitive strokes. For simpler use cases, the example above uses [getSvgPathFromPoints](?) directly.
