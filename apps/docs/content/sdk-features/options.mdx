...
A `createShapes` call that would exceed `maxShapesPerPage` is dropped in full without throwing. The editor emits a `max-shapes` event; with no listener registered it also logs a console warning, once per editor.
...
| `frameLoop`                  | 'auto'   | How the editor's tick loop runs          |
...
Setting `frameLoop` to `'manual'` starts no animation-frame loop; advance time yourself with `editor.emit('tick', elapsedMs)`. This is mainly useful for headless editors and tests.
...
See [Clipboard](/sdk-features/clipboard) for more details on clipboard operations.
