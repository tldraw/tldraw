...
When you move or resize shapes, tldraw can snap them to key geometry on nearby shapes. This helps users achieve precise alignment without manual measurement.

The editor provides three types of snapping:

- **Bounds snapping** aligns edges, centers, and corners during movement and resizing
- **Handle snapping** connects endpoints to outlines and key points (like arrow tips to shape edges)
...
The [SnapManager](?) coordinates all snapping behavior. Access it at `editor.snapping`:

```typescript
// The two snap systems
editor.snapping.shapeBounds // BoundsSnaps - edge and center alignment
editor.snapping.handles // HandleSnaps - precise point connections

// Shared utilities
editor.snapping.getSnapThreshold() // Distance threshold (8px / zoom)
editor.snapping.getSnappableShapes() // Which shapes can be snapped to
editor.snapping.setIndicators(indicators) // Update visual snap lines
editor.snapping.clearIndicators() // Remove all snap indicators
```

The snap threshold adjusts dynamically based on zoom level (8 pixels divided by zoom level), so snapping feels consistent regardless of how far you've zoomed in or out.

### Snappable shape filtering

The `getSnappableShapes()` method determines which shapes can be snapped to:

- Excludes currently selected shapes (you don't snap to what you're dragging)
- Includes only shapes visible in the viewport (for performance)
- Respects shape utility `canSnap()` for opt-out behavior
- Includes frame shapes as snap targets
- Recurses into groups, snapping to children but not the group itself
...
Return an empty array to disable snapping to a shape entirely.
...
Gap snapping maintains consistent spacing between shapes. The system detects gaps between adjacent shapes and provides three types:

**Gap center snapping** centers the selection within a gap larger than itself, creating equal spacing on both sides.

**Gap duplication snapping** duplicates an existing gap on the opposite side of a shape. If two shapes have a 100px gap between them, dragging a third shape snaps to create another 100px gap.

**Adjacent gap detection** finds all adjacent gaps with matching lengths and displays them together, helping maintain consistent spacing across many shapes.
...
Shapes define what handles can snap to through [ShapeUtil.getHandleSnapGeometry](?):
...
Point snapping takes priority over align snapping. Snap points always have higher priority than outlines.
...
**Gaps indicators** (`type: 'gaps'`) display spacing between shapes with measurement lines at each gap. When multiple equal-sized gaps exist, all matching gaps are shown to highlight the consistent spacing pattern.
...
- `/packages/editor/src/lib/editor/managers/SnapManager/SnapManager.ts` - Main coordinator
- `/packages/editor/src/lib/editor/managers/SnapManager/BoundsSnaps.ts` - Bounds snapping logic
- `/packages/editor/src/lib/editor/managers/SnapManager/HandleSnaps.ts` - Handle snapping logic

## Related examples

- **[Custom bounds snapping](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/bounds-snapping-shape)** - Create shapes with custom snap geometry so they snap to specific points, like playing cards that stack with visible icons.
- **[Custom relative snapping](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/custom-relative-snapping)** - Customize snapping behavior between shapes using getBoundsSnapGeometry.
