...
The available geometric forms are: `rectangle`, `ellipse`, `triangle`, `diamond`, `pentagon`, `hexagon`, `octagon`, `star`, `rhombus`, `rhombus-2`, `oval`, `trapezoid`, `arrow-right`, `arrow-left`, `arrow-up`, `arrow-down`, `x-box`, `check-box`, `cloud`, `heart`
...
The embed shape displays interactive content from external services within an iframe. The embed system recognizes URLs from supported services and converts them to their embeddable equivalents. When an embed shape is locked, you can interact with the content (play videos, use embedded applications) without accidentally moving the shape.
...
Embed shapes don't have per-instance configuration options, but you can customize the set of supported embed types globally using `EmbedShapeUtil.setEmbedDefinitions()`.
