...
| `highlightColors`   | `Record<string, Record<string, { srgb, p3 }>>` | (built-in palette) | Per-theme, per-color highlight colors in sRGB and Display P3. |

The highlight's `underlayOpacity` (default `0.82`) and `overlayOpacity` (default `0.35`) are display values that can be overridden via `getDisplayValueOverrides`.
...
| `frameColors`    | `Record<string, Record<string, { headingStroke, headingFill, stroke, fill, text }>>` | (built-in palette) | Per-theme, per-color frame colors. |
