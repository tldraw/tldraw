...
To remove built-in palette colors, augment the `TLRemovedThemeColors` interface. Any key you add will be omitted from the theme's color palette type, so TypeScript will no longer expect it:
...
```typescript
const { 'light-violet': _, 'light-blue': __, 'light-green': ___, 'light-red': ____, ...kept } =
	DEFAULT_THEME.colors.light
```
