		...
		<Tldraw
			onMount={(editor) => {
				...
				editor.updateTheme('default', (theme) => {
					theme.colors.light.black.solid = 'navy'
					return theme
				})
			}}
		/>
...
> Note: The `background` property is used by shapes and exports to match the canvas background color, but the actual visible canvas background is set separately via the `--tl-color-background` CSS variable. If you customize `background` in your theme, make sure the CSS variable matches — otherwise exports and shape rendering may use a different background color than what's shown on the canvas.
...
Use [Editor#updateTheme](?) with a callback to override specific colors in a theme. The callback receives a deep copy of the theme, so you can mutate it directly:
...
You can also pass a partial theme object to merge:
