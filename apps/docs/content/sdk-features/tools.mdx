...
- **[Sticker (shape, tool, and bindings)](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/shapes/tools/sticker)** - A custom tool that places emoji stickers on the canvas, demonstrating how to extend StateNode with child states and hand off to the select tool for drag-to-place interactions.
