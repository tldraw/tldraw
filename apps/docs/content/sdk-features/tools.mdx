...
completeness: 8
accuracy: 7
notes: "Strong architecture explanation with good examples. Fixed incorrect custom root state pattern - use tools prop instead of extending RootState. Event handlers and state machine mechanics accurately documented. Example links valid."
---
...
editor.setTool(MyTool)

// Remove a tool dynamically
editor.removeTool(MyTool)

// Check if you need to switch away from a removed tool
if (editor.getCurrentToolId() === 'my-tool') {
	editor.setCurrentTool('select')
}
```

When removing a tool, check whether the user is currently using it. If so, transition to a different tool like select to avoid leaving the editor in an invalid state.
