...
completeness: 9
accuracy: 9
notes: 'Clear structure with direct opening. Added connection to snapping system. Documented center, area, and toSimpleSvgPath(). Improved Point2d documentation. Added TransformedGeometry2d limitation note.'
---
...
Each [ShapeUtil](?) implements a [getGeometry](?) method that returns a [Geometry2d](?) instance. The editor calls this method to calculate bounds, test hits, find intersections, and measure distances.
...
Geometry also powers [snapping](/docs/snapping). When you drag shapes, the snapping system uses geometry to find edges, centers, and corners to align to. Custom shapes can provide additional snap points by implementing [getBoundsSnapGeometry](?).
