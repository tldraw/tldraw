...
<Callout type="warning">
	Without a license key that includes commenting, the commenting UI disappears in production: the
	toolbar button, the `C` and `Shift+C` shortcuts, and the comments layer all render nothing. The
	rest of the editor is unaffected, so the symptom is a canvas you can draw on with no way to
	comment on it. The feature is fully enabled in development, so this only shows up once you deploy.
	To branch on it yourself — hiding your own commenting affordances to match — read
	`useCommentingEnabled()`. See [License key](/sdk-features/license-key).
</Callout>

## Placing comments

`commentToolOverrides` puts the comment tool in the toolbar with the `C` shortcut. With the tool active, clicking empty canvas starts a thread anchored to that point, and clicking a shape anchors a comment to the shape. You can also turn on region comments, where dragging the tool out covers an area. Either way the click only opens a composer: nothing is written to the store until the comment is posted.
...
In the built-in layer, resolve and delete-thread sit in the thread's header, and the edit and delete controls for a single comment appear only on the author's own comments by default. You can customize what a client is able to control and write with comments, as [Syncing comments](#syncing-comments) covers.
...
Options are fixed once the tool is registered, so this is static configuration only. Live values, like the current user and the author resolver, stay as props on `CanvasComments`. Calls to `configure` can be chained, and each one layers over the last.
...
Comment writes are not undoable by default. The `history` option governs all of them, including posting, replying, editing, resolving, and deleting, and it defaults to `'ignore'`.
...
<Callout type="warning">
	`canComment` hides UI. It doesn't enforce anything. Comment records carry a client-supplied
	`createdBy` and `authorId`, and a client can write whatever your sync server accepts. Enforce
	permissions in your server's record authorization, and verify author ids against the session's
	authenticated identity. See [Syncing comments](#syncing-comments).
</Callout>
...
CommentTool.configure({
	enableRegions: true,
	regionReveal: 'pin-hover',
	regionMove: 'both',
	regionResize: 'edges',
})
```

`regionReveal` decides when the dashed box and its handles show: while the pointer is inside the region, while its pin is hovered, or only while the thread is open. `regionMove` and `regionResize` decide how it's dragged and resized, and `regionPinCorner` sets which corner the pin sits on. A region anchor also remembers the corner where its creating drag was released.
...
| Helper                 | Description                       |
| ---------------------- | --------------------------------- |
| `getCommentThreads`    | Every thread in the store.        |
| `getComments`          | Every comment in the store.       |
| `getCommentRecord`     | One record by id, or `undefined`. |
| `putCommentRecords`    | Write threads and comments.       |
| `removeCommentRecords` | Remove them by id.                |

Those reads are non-reactive. In React use the hooks instead: `useCommentThreads`, `useComments`, and `useThreadComments` for one thread's replies, oldest first.
...
To write a thread yourself, whether you're seeding a document with review notes or importing comments from another system, use the record helpers:
...
An unserved request is inert, so it's safe to set one before the records have synced in. For a thread you already hold, `focusThread(editor, thread)` centers and opens it directly.
...
The work is precomputed once per comment add, remove, or move, so per-frame camera changes cost a single walk over a sorted event table. If you're building your own pin layer, `computeClusterTable` and `createClusterRuntime` are exported. See the [comment clustering example](/examples/collaboration/comment-clustering).
...
This is also where per-record permissions belong. The lane's write access is all or nothing. Rules like "only the author may edit a comment" or "only the thread's creator may delete it" are yours to enforce as your server validates each incoming record against the session's identity.
...
| `CommentsList`, `EmptyState`             | A list of threads and its empty state.                      |
...
| `regionPinCorner` | `{x: 1, y: 1}` | Which corner of a region its pin sits on.                           |
| `regionReveal`    | `'pointer'`    | When a region's box shows: `'pointer'`, `'pin-hover'`, or `'open'`. |
| `regionMove`      | `'pin'`        | How a region moves: by its `'pin'`, its `'body'`, or `'both'`.      |
| `regionResize`    | `'corners'`    | A region's resize affordance: `'corners'`, `'edges'`, or `'none'`.  |
...
| `clusterCullMargin`      | `120`   | Screen-pixel margin by which the viewport is inflated when culling badges. |
| `clusterSplitZoomFactor` | `1.05`  | How far past a cluster's split zoom to land when expanding it.             |
