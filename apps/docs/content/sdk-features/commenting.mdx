...
<Callout type="warning">
	...
	production, the comment tool hands itself back to select, and comment records aren't written. The
	feature is fully enabled in development, so a missing key shows up as a blank canvas after you
	deploy — each refused action warns once in the console saying so. See [License
	key](/sdk-features/license-key).
</Callout>
