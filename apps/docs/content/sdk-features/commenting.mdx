...
`CanvasComments` needs two things, and both are about identity. `currentUserId` is the id of the person commenting, or `null` for a viewer who can read comments but not post them. `resolveAuthor` maps an author id to a `CommentAuthor`: a `name`, an optional `color` for their avatar and pin, and an optional `image`. Return `undefined` for an id you can't resolve and the layer falls back to an anonymous user. Pass them as props, or, once you're mounting more than one commenting surface, supply them once with [`CommentingProvider`](#sharing-the-identity).
...
Options are fixed once the tool is registered, so this is static configuration only. Live values, like the current user and the author resolver, are the `CommentingIdentity` instead — props on a surface, or a `CommentingProvider` around them. Calls to `configure` can be chained, and each one layers over the last.
...
Every commenting surface reads the same host wiring — the `CommentingIdentity`: who the viewer is, how author ids become names, read status, and the mention roster. Pass it as props on a single surface, as above, or supply it once with `CommentingProvider` and let each surface beneath inherit it.
...
A field left unset inherits; `null` is a value, not an omission. `useCommentingIdentity` reads whatever is in force, for surfaces you build yourself out of the parts.
