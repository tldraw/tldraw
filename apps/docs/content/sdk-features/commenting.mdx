...
`CanvasComments` needs two things, and both are about identity. `currentUserId` is the id of the person commenting, or `null` for a viewer who can read comments but not post them. `resolveAuthor` maps an author id to a `CommentAuthor`: a `name`, an optional `color` for their avatar and pin, and an optional `image`. Return `undefined` for an id you can't resolve and the layer falls back to an anonymous user. Pass them as props, or, once you're mounting more than one commenting surface, supply them once with [`CommentingProvider`](#sharing-the-commenting-context).
...
Options are fixed once the tool is registered, so this is static configuration only. Live values, like the current user and the author resolver, are the `CommentingContext` instead — props on a surface, or a `CommentingProvider` around them. Calls to `configure` can be chained, and each one layers over the last.
...
## Sharing the commenting context

Every commenting surface reads the same host wiring — the `CommentingContext`: who the viewer is, how author ids become names, read status, and the mention roster. Pass it as props on a single surface, as above, or supply it once with `CommentingProvider` and let each surface beneath inherit it.

A surface's own props still win over the provider, which is how one surface can differ from the rest. A session that may read comments but not write them passes `currentUserId={null}` to the layer to suppress composing, while the provider keeps the real id for the sidebar's "only your threads" filter:

```tsx
<CommentingProvider currentUserId={userId} resolveAuthor={resolveAuthor}>
	<CanvasComments currentUserId={canComment ? userId : null} />
	<CanvasCommentsSidebar />
</CommentingProvider>
```

A field left unset inherits; `null` is a value, not an omission. `useCommentingContext` reads whatever is in force, for surfaces you build yourself out of the parts.
...
An unserved request is inert, so it's safe to call before the records have synced in. `useRevealThreadPending` returns the id of a request that hasn't been served yet, which is how you notice a deep link to a comment that no longer exists — give it a grace period first, since a request also sits there while its records are still arriving.
