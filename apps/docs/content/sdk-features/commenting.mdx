---
title: Commenting
created_at: 07/27/2026
updated_at: 07/27/2026
keywords:
  - comment
  - comments
  - commenting
...
Its open state is a signal you drive, so the toggle lives wherever your app wants it:
...
Unread state drives the pin badges and the sidebar's unread filter. Without `isCommentUnread`, both are hidden. `onPostComment` fires after any comment is posted, which is where notifications and mention emails belong.
...
| `dragHistory` | `history`  | History mode for pin drags specifically.         |
