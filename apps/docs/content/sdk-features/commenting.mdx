...
This is also where per-record permissions belong. The lane's write access is all or nothing. Rules like "only the author may edit a comment" or "only the thread's creator may delete it" are yours to enforce as your server validates each incoming record against the session's identity. Whatever you decide there is the real rule: [`canModifyComment`](#who-can-edit-and-delete) only decides which affordances the UI offers, so widen the two together — a moderator who may remove anyone's comment needs the server to accept the write as well as the client to offer it.
