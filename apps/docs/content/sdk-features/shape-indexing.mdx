...
completeness: 8
accuracy: 8
notes: 'Clear, direct explanations with proper IndexKey casting. Code examples match API. Layer panel example link should be verified. Minor: TLShapeId not imported in reorder snippets.'
---
...
const between = getIndexBetween('a1' as IndexKey, 'a3' as IndexKey) // 'a2'

// Generate indices above or below an existing index
const above = getIndexAbove('a1' as IndexKey) // 'a2'
const below = getIndexBelow('a2' as IndexKey) // 'a1'

// Generate multiple indices at once
const multipleAbove = getIndicesAbove('a0' as IndexKey, 3) // ['a1', 'a2', 'a3']
const multipleBelow = getIndicesBelow('a2' as IndexKey, 2) // ['a1', 'a0V']
const multipleBetween = getIndicesBetween('a0' as IndexKey, 'a2' as IndexKey, 2) // ['a0V', 'a1']
