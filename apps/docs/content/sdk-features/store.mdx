...
completeness: 9
accuracy: 9
notes: "Strong doc. Links to side-effects for lifecycle hooks. Code snippets accurate, API matches source."
---
...
The reactive `get()` integrates with tldraw's signals system. When you call it inside a tracked component or computed, the component re-renders when that record changes.
...
The `update()` helper is more convenient for single-record updates:
...
Subscribe to store changes with `listen()`. The callback receives a diff describing what changed:
...
The store provides indexed queries for efficient lookups:
...
Batch multiple changes into a single update with `atomic()`:
...
For expensive derived data, use a computed cache:
...
Most of the time you use the store through the editor. But you can create a standalone store for testing or headless scenarios:
...
Creating your own store is useful when you need to load data before mounting the editor or share a store between multiple components.
