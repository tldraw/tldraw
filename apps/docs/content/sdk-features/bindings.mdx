...
- **[Sticker (shape, tool, and bindings)](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/shapes/tools/sticker)** - Stickers that stick to other shapes and follow them when moved. Demonstrates `onAfterChangeToShape` for position updates, plus creating and removing bindings from a shape's translate callbacks.
