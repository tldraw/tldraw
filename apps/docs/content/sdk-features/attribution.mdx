...
The built-in note shape tracks who last edited its text. Whenever a user types in a note, `NoteShapeUtil` sets the `textLastEditedBy` prop to the current user's ID via `editor.getAttributionUserId()`. Clearing the text resets it to `null`. The note renders that editor's name as a small label in the corner.

The label is rendered by the note shape util's `AttributionComponent` option, which you can override or hide with `NoteShapeUtil.configure`, passed through the `shapeUtils` prop. This applies on the canvas and in image exports:
...
;<Tldraw
	shapeUtils={[
		NoteShapeUtil.configure({
			AttributionComponent: ({ firstName, color }) => (
				<div className="tl-note__attribution" style={{ color }}>
					✍️ {firstName}
				</div>
			),
		}),
