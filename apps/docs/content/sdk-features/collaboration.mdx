...
The `users` store is also used for shape attribution — the same `getCurrentUser()` identity stamps shapes with user IDs. Provide a `resolve` method to look up other users by ID:
