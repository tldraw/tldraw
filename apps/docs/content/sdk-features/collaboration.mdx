...
import { useSyncDemo } from '@tldraw/sync'
import { Tldraw } from 'tldraw'
import { MyCustomShapeUtil, myCustomShapeProps } from './MyCustomShape'
...
The sync system lives in these packages:

- `packages/sync/src/useSync.ts` — The main `useSync` hook
- `packages/sync/src/useSyncDemo.ts` — The `useSyncDemo` hook for the demo server
- `packages/sync-core/src/lib/TLSyncClient.ts` — Client-side sync protocol implementation
- `packages/sync-core/src/lib/TLSocketRoom.ts` — Server-side room management
