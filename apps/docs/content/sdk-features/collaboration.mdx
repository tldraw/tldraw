...
By default, users get a random name and color from localStorage. To customize this, pass a `users` store:
...
The `users` store is also used for shape attribution — the same `getCurrentUser()` identity stamps shapes with `createdBy` and `updatedBy` user IDs. Provide a `resolve` method to look up other users by ID:

```tsx
const users: TLUserStore = {
	getCurrentUser: () => myAuth.currentUser,
	resolve: (userId) => myUserCache.get(userId) ?? null,
}
```
