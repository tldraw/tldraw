...
Both hover and hint indicators use the same selection color (`--tl-color-selected` CSS variable). The editor renders them on the canvas using the shape's `getIndicatorPath` implementation.
