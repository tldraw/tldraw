...
completeness: 8
accuracy: 8
notes: "Clear structure with practical examples. Fixed: arrow connection example now correctly uses bindings API; added missing source parameters to all handler signatures."
---
...
const cleanup = editor.sideEffects.registerAfterDeleteHandler('shape', (shape, source) => {
	// Clean up bindings involving deleted shape
