...
completeness: 9
accuracy: 9
notes: 'Improved accuracy of TLClickEventInfo type. Fixed first code example to be complete. Clarified state machine behavior.'
---
...
Tools can handle [onDoubleClick](?), [onTripleClick](?), and [onQuadrupleClick](?) events. Each receives a [TLClickEventInfo](?) object with details about the click.
