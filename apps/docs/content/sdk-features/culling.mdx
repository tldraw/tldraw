...
completeness: 6
accuracy: 9
notes: "Good overview, but missing full code example and user-facing API usage."
---
...
In the React canvas, culled shapes remain in the DOM but have their `display` property set to `none` and so incur no rendering cost. The system uses incremental derivations to efficiently track visibility changes as the camera moves or shapes are modified. Performance remains consistent even with thousands of shapes on the canvas.

## How it works

The culling system operates through a two-layer approach that balances performance with user experience.

The first layer, exposed through `editor.getNotVisibleShapes()`, identifies all shapes whose page bounds don't intersect with the viewport. The calculation iterates through every shape on the current page and performs a simple bounds collision check. This comparison is fast enough to run reactively.
...
- **[Camera options](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/camera-options)** - Configure camera constraints and bounds that affect which shapes are visible and culled.
