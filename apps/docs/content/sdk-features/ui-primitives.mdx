...
completeness: 7
accuracy: 8
notes: "Clear examples with good API coverage. The TldrawUiMenuCheckboxItem onSelect signature was incorrect (missing source parameter). Example URLs may need verification against live routing. Could add more depth on theming integration and custom icon registration."
---
...
;<TldrawUiMenuCheckboxItem
	...
	label="Snap to grid"
	checked={snapEnabled}
	onSelect={(source) => setSnapEnabled(!snapEnabled)}
/>
```
