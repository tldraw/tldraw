...
completeness: 8
accuracy: 7
notes: "Good coverage of InputsManager APIs. Event flow section slightly misleading: click manager dispatch only happens for pointer events. Event type list incomplete (missing TLPinchEventInfo, TLClickEventInfo). Code snippets accurate."
---
...
3. The editor transforms it into a typed event info object (`TLPointerEventInfo`, `TLKeyboardEventInfo`, `TLWheelEventInfo`, `TLPinchEventInfo`, or `TLClickEventInfo`)
4. The editor calls `updateFromEvent()` on the `InputsManager` to update input state
5. For pointer events, the editor dispatches to the click manager for multi-click detection
...
- **[Reactive inputs](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/reactive-inputs)** - Display pointer positions, velocity, and other input state reactively as you interact with the canvas.
- **[Canvas events](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/canvas-events)** - Log all pointer, keyboard, and wheel events to understand the event flow.
