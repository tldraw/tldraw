...
completeness: 8
accuracy: 8
notes: 'Well-structured coverage of signals API. Good code examples with correct API usage. Debugging section could show actual console output. Links to examples work. All API references verified against source.'
---

The tldraw SDK uses a signals-based reactive system for state management. Signals provide automatic dependency tracking, lazy evaluation, and efficient updates. When state changes, only the parts of your application that depend on that state will update.
...
Run effects tied to component lifecycle:

```tsx
import { useReactor, useQuickReactor } from '@tldraw/state-react'

function SelectionLogger({ editor }) {
	// Throttled to animation frames — good for visual updates
	useReactor(
		'log selection',
		() => {
			console.log('Selection:', editor.getSelectedShapeIds())
		},
		[editor]
	)
...
The `whyAmIRunning` function helps trace what triggered an update:

```ts
import { whyAmIRunning } from '@tldraw/state'

react('debug effect', () => {
	whyAmIRunning() // Prints dependency tree to console
	return someComputed.get()
})
```
