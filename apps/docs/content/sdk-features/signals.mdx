...
Tldraw uses a signals-based reactive system for state management. Signals provide automatic dependency tracking, lazy evaluation, and efficient updates. When state changes, only the parts of your application that depend on that state will update.
