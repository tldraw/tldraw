...
completeness: 8
accuracy: 9
notes: "Strong architectural overview with good cross-linking. All API calls verified correct."
---
...
editor.root.path // 'root.select.idle', 'root.draw.drawing', etc.
...
The [Editor#inputs](?) object tracks the user's current input state: cursor position, pressed keys, drag state, and more.
...
// Modifier keys (with a small delay after release)
...
The editor maintains per-instance state in a [TLInstance](?) record. This includes which page is current, whether the editor is in readonly mode, the current tool, and UI state.
