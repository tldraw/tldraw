...
All export methods accept a [TLImageExportOptions](?) object:
...
| `padding`             | Space in pixels around the shape bounds. Defaults to 32. Skipped when exporting a single frame or shapes within an image.                                                                              |
...
| `preserveAspectRatio` | The SVG `preserveAspectRatio` attribute. Only applies to SVG exports.                                                                                                                                  |
...
- [Custom shape SVG export](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/toSvg-method-example) - Define how custom shapes render when exported using [toSvg](?) and [toBackgroundSvg](?).
