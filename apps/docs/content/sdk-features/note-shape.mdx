...
| `textLastEditedBy`   | `string \| null`                | ID of the user who last edited the note's text                                   |
...
| `AttributionComponent` | `ComponentType<TLNoteShapeAttributionProps>` \| `null` | `DefaultNoteShapeAttribution` | Component for the attribution badge. Set to `null` to hide it, or provide your own to replace it. |
...
Notes show the name of the user who last edited their text as a small badge in the corner. This badge is rendered by the `AttributionComponent` option above, which you can override or hide with `NoteShapeUtil.configure`—it applies on the canvas and in image exports. See [attribution](/sdk-features/attribution) for details.
...
- [Attribution](/sdk-features/attribution) — How notes display "last edited by" labels in multiplayer sessions
