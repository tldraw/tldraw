...
Notes show the name of the user who first edited their text as a small badge in the corner. This badge is rendered by the `NoteShapeAttribution` component, which you can override or hide through the `components` prop—it applies on the canvas and in image exports. See [attribution](/sdk-features/attribution) for details.
