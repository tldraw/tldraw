...
| `noteColors` | `Record<string, Record<string, { fill, text }>>` | (built-in palette) | Per-theme, per-color fill and text colors for note shapes. |
| `noteBorder` | `Record<string, string>` | (built-in) | Per-theme border color for note shapes. |
