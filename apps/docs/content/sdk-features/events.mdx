...
completeness: 7
accuracy: 8
notes: "Good structure and clear explanations. Fixed store.listen() filter options (source/scope). Input events table omits zoom type (minor). Code examples are correct and API references accurate."
---

The editor emits events that let you respond to user interactions, state changes, and lifecycle moments. You subscribe using the standard `on()` and `off()` methods inherited from EventEmitter. Events range from low-level input (pointer moves, key presses) to high-level changes (shapes created, camera moved). Use them to build analytics, sync external state, or extend editor behavior.
...
Shape events fire when shapes change. These are convenience events—you can also use the `change` event to track all store modifications.
...
UI events track actions like selecting tools, grouping shapes, toggling dark mode, and zooming. They fire regardless of whether the action came from a click or keyboard shortcut.
