...
The `id` uniquely identifies the action. The `label` is a translation key (or an object mapping menu types to different labels). The `kbd` property accepts shortcut strings like `'cmd+g,ctrl+g'` for cross-platform support. The `readonlyOk` flag indicates whether the action works in readonly mode. The `checkbox` flag marks toggle-style actions that show a checkmark when active.
...
Shortcuts are disabled when focus is inside a text input. Actions marked with `isRequiredA11yAction: true` bypass this check for accessibility purposes.
