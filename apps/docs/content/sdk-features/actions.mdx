...
To bind the `+` key itself, double the trailing plus or use an atomic token: `cmd++` or `cmd+[[+]]`. That matches an unshifted `+` key (German, Nordic and similar layouts, or the numpad). On layouts where `+` is shift plus another key, such as US, write `cmd+shift++`, which is the same binding as `cmd+shift+=`. Shortcuts match the character the browser reports for the key press, not the physical keycap, with two exceptions. Shifted number-row keys match by position, so write them as `shift+<digit>` on every layout. When the reported character is non-ASCII (Cyrillic, Greek, macOS Option dead keys), matching falls back to the physical key's US QWERTY character, so `cmd+z` still works on a Russian layout. With shift held, the shifted and unshifted US spellings are the same binding, so `shift+:` and `shift+;` both fire when the browser reports `:` with shift held. The characters `!`, `?` and `$` are reserved as legacy modifier markers and cannot be used as keys.
