...
```tsx
const myLicenseKey = 'tldraw-***********************'

function App() {
	return <Tldraw licenseKey={yourLicenseKey} />
}
```

License keys are validated **on the client**. You can use them offline.

It safe for license keys to be public. Business and Hobby license keys will only work on pre-agreed domains. Trial license keys will work on any domain.
