...
One sentence describes the whole SDK. Zoom in and it becomes five areas, then eleven subsystems, then
forty-eight modules — editor internals, default shapes, the store, signals, sync.
...
each leaf — and that needs no special casing. A corpus without detail simply has one fewer level.
