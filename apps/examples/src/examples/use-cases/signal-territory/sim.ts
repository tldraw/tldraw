import { Owner, Peg, Point, publishScores, Pulse, Strand, uidPulse, World } from './game-state'
...
const MAX_HOPS = 14 // pegs a pulse can chain through before it fizzles
const PEG_EPS = 1 // a pinned end this close to a peg counts as attached
const EMIT_STRENGTH = 1.0 // strength of a freshly emitted pulse
...
// applying charge deposits and decay.
export function stepSignals(world: World) {
	// Decay all charge and strand highlights first.
	...
	// Emit from each player's source on a shared timer.
	world.emitTimer++
	...
	for (const pulse of world.pulses) {
		...
		const onward = strandsAtPeg(world, peg).filter(({ strand: s2 }) => s2.id !== strand.id)
