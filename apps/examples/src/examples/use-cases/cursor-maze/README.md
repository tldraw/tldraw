...
Every cursor also leaves a short, laser-style trail in that player's own color. It's drawn to a plain canvas that fills the editor, fed each frame from the cursor positions the game already has — your own collision-resolved position plus every collaborator's synced cursor — so the trail is a pure local effect that adds nothing to what's synced. Each point fades and thins as it ages, then drops out, giving the tapering comet look without any per-frame React work.
