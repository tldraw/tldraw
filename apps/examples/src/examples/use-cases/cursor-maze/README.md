...
Every cursor also leaves a laser-style trail in that player's own color. It's drawn to a plain canvas that fills the editor, fed each frame from the cursor positions the game already has — your own collision-resolved position plus every collaborator's synced cursor — so the trail is a pure local effect that adds nothing to what's synced. Each player's recent points become one filled ribbon whose width tapers to nothing at the tail, so the fade is geometric rather than alpha-based. Painting it as a single `fill()` keeps even a self-crossing loop solid — no overlapping-segment dots the way a stack of translucent strokes would leave, and no straight-axis gradient that can't follow a curl — and the oldest points drop off the tail as they age.
