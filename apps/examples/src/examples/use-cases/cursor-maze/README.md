...
Every cursor also leaves a laser-style trail in that player's own color. It's drawn to a plain canvas that fills the editor, fed each frame from the cursor positions the game already has — your own collision-resolved position plus every collaborator's synced cursor — so the trail is a pure local effect that adds nothing to what's synced. Each player's recent points become one smooth path stroked with a single gradient, transparent at the tail and solid at the cursor; drawing it as one stroke (rather than a segment per pair of points) keeps it a clean streak, and the oldest points drop off the tail as they age.
