...
import { buildPreview$, frame$, getWorld, GRID, Owner, pegOwner, PLAYER_COLOR } from '../game-state'
import { pulsePos } from '../sim'

interface TLWarsOverlay extends TLOverlay {
	props: { frame: number }
}

type ThemeColors = (typeof DEFAULT_THEME.colors)['light']

// Draws the whole board on one canvas layer: a faint Go-board grid, pegs tinted
// by owner, strands in their flowing owner's color, energy pulses, the two home
// sources, and the player's build rubber-band preview. A single overlay whose
// `frame` prop is bumped each tick, so the reactive render re-runs every frame
// and reads live world state.
export class WarsOverlayUtil extends OverlayUtil<TLWarsOverlay> {
	static override type = 'sw-wars'
	...
	override getOverlays(): TLWarsOverlay[] {
		return [{ id: 'sw-wars:board', type: 'sw-wars', props: { frame: frame$.get() } }]
	}
	...
	override render(ctx: CanvasRenderingContext2D): void {
		...
		// Faint Go-board grid lines connecting adjacent pegs.
		...
		for (let r = 0; r < GRID.rows; r++) {
			...
			ctx.lineTo(GRID.x0 + (GRID.cols - 1) * GRID.spacing, y)
		}
		for (let c = 0; c < GRID.cols; c++) {
			const x = GRID.x0 + c * GRID.spacing
			ctx.moveTo(x, GRID.y0)
			ctx.lineTo(x, GRID.y0 + (GRID.rows - 1) * GRID.spacing)
		}
		ctx.stroke()
		ctx.globalAlpha = 1
		...
		for (const peg of world.pegs) {
			...
			if (owner) {
				...
				ctx.beginPath()
				ctx.arc(peg.x, peg.y, px(16), 0, Math.PI * 2)
				ctx.fill()
				// The peg block itself in the owner's color.
				ctx.globalAlpha = 0.5 + 0.5 * strength
				block(ctx, peg.x, peg.y, px(6), px(2))
				ctx.fill()
			} else {
				ctx.globalAlpha = 0.16
				ctx.fillStyle = theme.text
				block(ctx, peg.x, peg.y, px(4), px(1.2))
				ctx.fill()
			}
		}
		ctx.globalAlpha = 1
		...
		for (const strand of world.strands) {
			...
			ctx.lineWidth = px(strand.flow ? 4 : 3)
			ctx.beginPath()
			...
			// Free (cut/dangling) ends get a small dim nub.
			ctx.globalAlpha = 0.6
			for (const end of [strand.points[0], strand.points[strand.points.length - 1]]) {
				if (end.pinned) continue
				block(ctx, end.x, end.y, px(7), px(2))
		...
		// Home sources — large blocky emitters in each player's color.
		for (const owner of ['a', 'b'] as Owner[]) {
			const peg = world.pegs.find((p) => p.id === world.sources[owner])
			if (!peg) continue
			block(ctx, peg.x, peg.y, px(18), px(5))
			ctx.fillStyle = colorOf(owner)
			ctx.fill()
			ctx.lineWidth = px(3)
		...
		for (const pulse of world.pulses) {
			...
			ctx.lineWidth = px(8 * size)
			strokeAlong(ctx, strand, pulse.dir, pulse.dist - px(26 * size), pulse.dist, px(7))

			// Core: a short bright packet at the head, in the theme's white.
			ctx.globalAlpha = 0.5 + 0.5 * size
			ctx.strokeStyle = theme.white.solid
			ctx.lineWidth = px(3.5 * size)
			strokeAlong(ctx, strand, pulse.dir, pulse.dist - px(11 * size), pulse.dist, px(6))
		}
		ctx.globalAlpha = 1

		// Build preview — the player's rubber-band line while dragging from one of
		// their pegs. Green + solid when the current target is a valid/affordable
		// drop; dim red + dashed otherwise.
		const preview = buildPreview$.get()
		if (preview) {
			ctx.save()
			if (preview.valid) {
				ctx.globalAlpha = 0.9
			...
			ctx.stroke()
			// Endpoint ring at the target.
			ctx.setLineDash([])
			ctx.beginPath()
			ctx.arc(preview.to.x, preview.to.y, px(8), 0, Math.PI * 2)
			ctx.stroke()
			ctx.restore()
		}
		ctx.globalAlpha = 1
