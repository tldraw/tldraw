...
The toolbar is wrapped in `track()` so the active button follows `editor.getCurrentToolId()`. Note that `hideUi` doesn't remove the context menu; see the exploded example for how to omit it.
