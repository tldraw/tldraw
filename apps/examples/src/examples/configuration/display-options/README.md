...
This example configures `GeoShapeUtil` so that locked shapes are filled red, ellipses use a monospace italic label, and every geo shape has a 10px stroke. Try locking the rectangle (select it, then use the context menu) to see the fill change, or change the size style and notice that the stroke width stays the same.
