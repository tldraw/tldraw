...
The commenting layer doesn't decide what's unread or what's worth notifying someone about — it takes both from the host. `isCommentUnread` reports read status, `onCommentRead` records a receipt for every unread comment a thread view displays, and `revealThread` jumps to a thread from anywhere outside the canvas.
