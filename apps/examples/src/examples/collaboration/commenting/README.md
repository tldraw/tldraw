...
`CanvasComments` takes a `currentUserId` (the signed-in user, or `null` for a read-only viewer) and a `resolveAuthor` function that maps author ids to display info (name, color, and an optional avatar image). The optional `getMentionSuggestions` prop supplies the `@`-mention roster, and configuring the tool with `CommentTool.configure({ enableRegions: true })` turns on drag-to-comment regions.
