...
`CanvasComments` takes a `currentUserId` (the signed-in user, or `null` for a read-only viewer) and a `resolveAuthor` function that maps author ids to display info (name, color, and an optional avatar image). The optional props here enable more of the toolkit: `regionOptions={{ enabled: true }}` turns on drag-to-comment regions, and `getMentionSuggestions` supplies the `@`-mention roster.
