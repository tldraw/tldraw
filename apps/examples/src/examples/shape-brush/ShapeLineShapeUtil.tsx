...
} from 'tldraw/src/lib/shapes/line/components/getLinePath'
import {
	getSvgPathForBezierCurve,
	getSvgPathForEdge,
	getSvgPathForLineGeometry,
} from 'tldraw/src/lib/shapes/line/components/svg'
import { ShapeFill } from 'tldraw/src/lib/shapes/shared/ShapeFill'
import { getDrawLinePathData } from 'tldraw/src/lib/shapes/shared/polygon-helpers'
import { STROKE_SIZES } from '../speech-bubble/SpeechBubble/SpeechBubbleUtil'

const handlesCache = new WeakMapCache<TLLineShape['props'], TLHandle[]>()

/** @public */
export class ShapeLineShapeUtil extends ShapeUtil<TLLineShape> {
	static override type = 'shape-line' as const
		...
		return (
			<>
				...
				{vertices.map((vertex, i) => {
					...
					const character = text[(i - 1) % text.length]
					return (
						<text key={`${shape.id}-${i}`} x={vertex.x} y={vertex.y} fontFamily="tldraw_draw">
							{character}
						</text>
					)
				})}
