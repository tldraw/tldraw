...
function GameRunner() {
	...
	useEffect(() => {
		...
		const onPointerDown = (e: PointerEvent) => {
			// No play once the output has starved out — press R to start over.
...
function Hud() {
	...
	const survived = useValue('survived', () => survived$.get(), [])
	const over = useValue('over', () => gameOver$.get(), [])
	return (
		<div
			style={{
				position: 'absolute',
		...
		>
			<span>Survived: {survived}s</span>
			<span>Signals delivered: {arrivals}</span>
			<span style={{ opacity: 0.6, fontWeight: 400, fontSize: 12 }}>
				keep the output fed · cut hot wires to shed load · drag a loose end onto a peg · R to reset
			</span>
			{over && (
				<span style={{ marginTop: 8, fontSize: 18, color: 'var(--tl-color-text)' }}>
					Output starved — game over. Press R to restart.
				</span>
			)}
		</div>
	)
...
export default function GridCascadeExample() {
	return (
