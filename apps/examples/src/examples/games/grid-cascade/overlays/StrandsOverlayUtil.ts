...
import { frame$, getWorld, Point, Strand } from '../game-state'
...
// Draws the whole board on one canvas layer: pegs, strands (tinted/thickened by
// heat, flashing when they burn), source/output nodes, energy pulses, free-end
// nubs, and the fading laser cut. A single overlay whose `frame` prop is bumped
// each tick, so the reactive render re-runs every frame and reads live state.
export class StrandsOverlayUtil extends OverlayUtil<TLStrandsOverlay> {
	static override type = 'gc-strands'
	...
	override getOverlays(): TLStrandsOverlay[] {
		return [{ id: 'gc-strands:board', type: 'gc-strands', props: { frame: frame$.get() } }]
	}
	...
	override render(ctx: CanvasRenderingContext2D): void {
		...
		// Strands — thick flat lines. As heat rises the colour pushes toward hot
		// orange then red and the line thickens; a burn spark flashes white.
		ctx.lineCap = 'round'
		ctx.lineJoin = 'round'
		const hot = theme.orange.solid
		const danger = theme.red.solid
		for (const strand of world.strands) {
			const base = (theme[strand.color as keyof ThemeColors] as { solid: string }).solid
			const h = Math.min(1, strand.heat)
			// Below ~0.4 stay near the base colour; above it ramp to orange→red.
			let color = base
			if (h > 0.4) color = mix(hot, danger, Math.min(1, (h - 0.4) / 0.6))
			if (h > 0.2) color = mix(base, color, Math.min(1, (h - 0.2) / 0.4))
			ctx.strokeStyle = color
			ctx.lineWidth = px(5 + 4 * h) // thicken as it loads up
			ctx.beginPath()
			...
			// Burn flash — a bright white overstroke that fades over the spark window.
			if (strand.spark > 0) {
				ctx.globalAlpha = Math.min(1, strand.spark / 8)
		...
		// The output ring pulses red the longer it has gone without a signal.
		const starveT = Math.min(1, world.starve / 240)
		for (const id of world.outputs) {
			station(id, mix(theme.orange.solid, theme.red.solid, starveT))
		}
