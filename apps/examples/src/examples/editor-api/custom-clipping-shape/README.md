...
The example defines a circle shape that clips its children to a polygon approximation of the circle. Try dragging shapes into and out of the circle, resizing it, or drawing new circles with the "Circle clip" tool (shortcut `c`). The "Disable clipping" button flips a shared atom that `shouldClipChild` reads, turning clipping off for every circle at once.
