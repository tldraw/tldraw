...
export class SimpleSelectToolUtil extends ToolUtil<SimpleSelectState, SimpleSelectConfig> {
	id = 'select' as const

	getDefaultConfig() {
		return {
			onDoubleClickCanvas: undefined,
		}
	}
	...
	override onEvent(info: TLEventInfo) {
		...
		switch (state.name) {
			case 'idle': {
				this.handleIdleStateEvent(state, info)
				break
			}
			case 'pointing_canvas': {
				this.handlePointingCanvasStateEvent(state, info)
				break
			}
			case 'pointing_handle': {
				this.handlePointingHandleStateEvent(state, info)
				break
			}
			case 'pointing_resize_handle': {
				this.handlePointingResizeHandleStateEvent(state, info)
				break
			}
			case 'pointing_rotate_handle': {
				this.handlePointingRotateHandleStateEvent(state, info)
				break
			}
			case 'pointing_crop_handle': {
				this.handlePointingCropHandleStateEvent(state, info)
				break
			}
			case 'pointing_selection': {
				this.handlePointingSelectionStateEvent(state, info)
				break
			}
			case 'pointing_shape': {
				this.handlePointingShapeStateEvent(state, info)
				break
			}
			case 'pointing_arrow_label': {
				this.handlePointingArrowLabelStateEvent(state, info)
				break
			}
			case 'cropping': {
				this.handleCroppingStateEvent(state, info)
				break
			}
			case 'crop_idle': {
				this.handleCropIdleStateEvent(state, info)
				break
			}
			case 'crop_pointing': {
				this.handleCropPointingStateEvent(state, info)
				break
			}
			case 'crop_translating': {
				this.handleCropTranslatingStateEvent(state, info)
				break
			}
			case 'dragging_handle': {
				this.handleDraggingHandleStateEvent(state, info)
				break
			}
			case 'editing_shape': {
				this.handleEditingShapeStateEvent(state, info)
				break
			}
			case 'resizing': {
				this.handleResizingStateEvent(state, info)
				break
			}
			case 'rotating': {
				this.handleRotatingStateEvent(state, info)
				break
			}
			case 'translating': {
				this.handleTranslatingStateEvent(state, info)
				break
			}
			case 'brushing': {
				this.handleBrushingStateEvent(state, info)
				break
			}
			case 'scribble_brushing': {
				this.handleScribbleBrushingStateEvent(state, info)
				break
			}
		}
	}

	handleIdleStateEvent(state: SimpleSelectState & { name: 'idle' }, info: TLEventInfo) {
		const { editor } = this
		...
		if (info.name === 'state_change') {
			...
			switch (info.target) {
				case 'canvas': {
					...
					if (!editor.inputs.shiftKey) {
						if (this.config.onDoubleClickCanvas) {
							this.config.onDoubleClickCanvas()
						} else {
			...
			switch (info.code) {
				case 'Enter': {
					...
					if (selectedShapes.every((shape) => editor.isShapeOfType<TLGroupShape>(shape, 'group'))) {
	...
	handlePointingCanvasStateEvent(
		state: SimpleSelectState & { name: 'pointing_canvas' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
	...
	handlePointingHandleStateEvent(
		state: SimpleSelectState & { name: 'pointing_handle' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
	...
	handlePointingResizeHandleStateEvent(
		state: SimpleSelectState & { name: 'pointing_resize_handle' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
	...
	handlePointingRotateHandleStateEvent(
		state: SimpleSelectState & { name: 'pointing_rotate_handle' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
		...
		} else if (info.name === 'complete' || info.name === 'interrupt' || info.name === 'cancel') {
			if (state.onInteractionEnd) {
				state.onInteractionEnd()
			} else {
				this.setState({ name: 'idle' })
	...
	handlePointingCropHandleStateEvent(
		state: SimpleSelectState & { name: 'pointing_crop_handle' },
		info: TLEventInfo
	) {
		const { editor } = this
	...
	handlePointingSelectionStateEvent(
		state: SimpleSelectState & { name: 'pointing_selection' },
		info: TLEventInfo
	) {
		const { editor } = this
	...
	handlePointingShapeStateEvent(
		state: SimpleSelectState & { name: 'pointing_shape' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
		...
		} else if (info.name === 'cancel' || info.name === 'interrupt' || info.name === 'complete') {
			this.setState({ name: 'idle' })
		} else if (info.name === 'double_click') {
	...
	handlePointingArrowLabelStateEvent(
		state: SimpleSelectState & { name: 'pointing_arrow_label' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
	...
	handleCroppingStateEvent(state: SimpleSelectState & { name: 'cropping' }, info: TLEventInfo) {
		const { editor } = this
		if (info.name === 'state_change') {
			if (info.from === state.name) {
				// exit
	...
	handleCropIdleStateEvent(state: SimpleSelectState & { name: 'crop_idle' }, info: TLEventInfo) {
		const { editor } = this
		if (info.name === 'state_change') {
			if (info.from === state.name) {
				// exit
	...
	handleCropPointingStateEvent(
		state: SimpleSelectState & { name: 'crop_pointing' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
	...
	handleCropTranslatingStateEvent(
		state: SimpleSelectState & { name: 'crop_translating' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
	...
	handleResizingStateEvent(state: SimpleSelectState & { name: 'resizing' }, info: TLEventInfo) {
		const { editor } = this
		...
		} else if (info.name === 'pointer_move' || info.name === 'key_down' || info.name === 'key_up') {
			this.updateResizingShapes(state)
	...
	handleDraggingHandleStateEvent(
		state: SimpleSelectState & { name: 'dragging_handle' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
	...
	handleRotatingStateEvent(state: SimpleSelectState & { name: 'rotating' }, info: TLEventInfo) {
		const { editor } = this
		if (info.name === 'state_change') {
			if (info.from === state.name) {
				// exit
	...
	handleTranslatingStateEvent(
		state: SimpleSelectState & { name: 'translating' },
		info: TLEventInfo
	) {
		const { editor } = this
		...
		if (info.name === 'state_change') {
			...
			if (state.isCloning && movingShapes.length > 0) {
				...
				const offset = Vec.Sub(currentAveragePagePoint, state.selectionSnapshot.averagePagePoint)
				if (!Vec.IsNaN(offset)) {
	...
	handleEditingShapeStateEvent(
		state: SimpleSelectState & { name: 'editing_shape' },
		info: TLEventInfo
	) {
		const { editor } = this
		if (info.name === 'state_change') {
	...
	handleScribbleBrushingStateEvent(
