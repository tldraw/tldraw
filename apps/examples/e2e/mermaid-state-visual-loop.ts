...
const outputDir = path.join(__dirname, 'test-results', 'mermaid-state-visual-loop')
...
async function main() {
	...
	try {
		...
		for (const [index, fixture] of mermaidStateLoopFixtures.entries()) {
		...
		process.stdout.write(`[mermaid:state:visual] report: ${reportPath}\n`)
		...
		if (failing.length) {
			console.error(
				`[mermaid:state:visual] ${failing.length} failing fixture(s) under ${
			...
			process.stdout.write('[mermaid:state:visual] all monitored fixtures passed\n')
		}
	} finally {
		await page.close()
		await context.close()
