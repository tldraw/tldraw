...
interface Bench {
	label: string
	// One ms/op sample per pass; the committed baseline is the median across them.
	measureHtml: number[]
	geometry: number[]
	resizeStep: number[]
}
...
test.describe('text measurement performance', () => {
	...
	test('measure / geometry / resize hot paths', async ({ page }, testInfo) => {
		...
		const results: Bench[] = await page.evaluate(
			async ({ iterations, passes }) => {
				...
				// Time each hot path once per pass; the caller medians across the passes. Vary the text
				// each iteration so the browser can't trivially reuse a cached layout.
				async function benchOne(
					...
					return { label, measureHtml, geometry, resizeStep }
				}
		...
		// Transpose into one sample set per pass, keyed "<label> <metric>", so recordPerfBaseline can
		// take the median across passes (dropping a slow pass) rather than averaging it in.
		const runs: Record<string, number>[] = []
		for (let p = 0; p < PASSES; p++) {
			const set: Record<string, number> = {}
			for (const r of results) {
				set[`${r.label} measureHtml`] = r.measureHtml[p]
				set[`${r.label} geometry`] = r.geometry[p]
				set[`${r.label} resizeStep`] = r.resizeStep[p]
			}
			runs.push(set)
		}
		...
		// Print a table (median ms/op) so the before/after-fix cost is easy to eyeball.
		const table = results
			.map(
				(r) =>
					`  ${r.label.padEnd(14)} measureHtml=${medianed[`${r.label} measureHtml`]}ms  geometry=${medianed[`${r.label} geometry`]}ms  resizeStep=${medianed[`${r.label} resizeStep`]}ms`
			)
			.join('\n')
		// eslint-disable-next-line no-console
		console.log(
			`\nText measurement (median of ${PASSES} runs, ${ITERATIONS} iters each):\n${table}\n`
		)
		test
			.info()
