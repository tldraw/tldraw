...
// Smoke test: every table example loads and renders its seeded table. Catches
// broken routes (wrong example folder/category) and import/render errors.

const SEEDED = ['table', 'table-structure', 'table-styling', 'table-affordances']
...
test.describe('table examples smoke', () => {
	for (const slug of SEEDED) {
		test(`/${slug} renders a seeded table`, async ({ page }) => {
			...
			// the seeded table + its cells render as shapes on the canvas
