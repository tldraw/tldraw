...
By design, the [`Editor`](/gen/editor/Editor)'s surface area is very large. Almost everything is available through it as a direct member of the class. Need to create a shape? Use [`editor.createShape()`](/gen/editor/Editor#createShape). Need to delete the shape? Use [`editor.deleteShape(editor.selectedShapeIds)`](/gen/editor/Editor#deleteShape). Need a sorted array of every shape on the current page? Use [`editor.currentPageShapes`](/gen/editor/Editor#currentPageShapes).
...
The editor holds the raw state of the document in its [`store`](/gen/editor/Editor#store). This is an in-memory database where data is stored as a flat table of JSON serializable "records". Each store has a unique `id`, a `typeName` that identifies what kind of the record it is, and a `scope` that describes what the record relates to.

> The store has the type of functionality that you'd expect to find in a database—methods like `put`, `get`, `update`, `remove`—as well as integrations with tldraw's other systems for state management, persistence, and side effects. In most cases, you shouldn't need to reference the store directly: for example, `Editor.createShapes` will call `store.put` when creating new shapes.

In tldraw, the store manages a finite set of records (affectionately typed as [`TLRecord`](gen/tlschema/TLRecord)):

| Name                                                      | typeName                | scope        |
| --------------------------------------------------------- | ----------------------- | ------------ |
| [`TLDocument`](gen/tlschema/TLDocument)                   | `'document'`            | `'document'` |
| [`TLPage`](gen/tlschema/TLPage)                           | `'page'`                | `'document'` |
| [`TLShape`](gen/tlschema/TLShape)                         | `'shape'`               | `'document'` |
| [`TLAsset`](gen/tlschema/TLAsset)                         | `'asset'`               | `'document'` |
| [`TLInstance`](gen/tlschema/TLInstance)                   | `'instance'`            | `'session'`  |
| [`TLCamera`](gen/tlschema/TLCamera)                       | `'camera'`              | `'session'`  |
| [`TLPointer`](gen/tlschema/TLPointer)                     | `'page'`                | `'session'`  |
| [`TLInstancePageState`](gen/tlschema/TLInstancePageState) | `'instance_page_state'` | `'session'`  |
| [`TLInstancePresence`](gen/tlschema/TLInstancePresence)   | `'instance_presence'`   | `'presence'` |
...
See the [document structure](#document-structure) section below.
...
export const SelectedShapeIdsCount = track(() => {
	...
	return <div>{editor.selectedShapeIds.length}</div>
})
```
...
export const CreatingBubbleToolUi = track(() => {
	...
	return <div data-isSelected={isSelected}>Creating Bubble</div>
})
```
