...
## Components
This is valid for versions under 2.0.0.

```tsx
function Example() {
  const component = { Cursor: UserCursor };
```

You need to import 'CursorComponent' library in '@tldraw/core'

```tsx
import { CursorComponent } from '@tldraw/core';
const UserCursor: CursorComponent<{ name: 'Anonymous' }> = ({
  color,
  metadata,
})
```
