...
async function writeConstantSetters() {
	...
	const constantFiles = [
		[
			join(REPO_ROOT, 'packages', 'editor', 'src', 'lib', 'settings.ts'),
			'DEFAULT_EDITOR_SETTINGS',
			'getEditorSettings',
			'updateEditorSettings',
		],
		[
			join(REPO_ROOT, 'packages', 'tldraw', 'src', 'lib', 'ui', 'settings.ts'),
	...
	for (const [
		...
		const tagLineIndex = lines.findIndex((line) => line.includes(AUTO_TAG))
		if (tagLineIndex === -1) throw new Error(`No ${AUTO_TAG} found in ${fileName}`)

		const relevantLines = lines.slice(0, tagLineIndex)
		...
		const autoGeneratedLines = [
			AUTO_TAG,
			'/** @public */',
			`export const ${defaultSettingsName} = {`,
			...
			'/** @public */',
			`export function ${updateFunctionName}(`,
			'	settings: {',
		...
		const newContent = [...relevantLines, ...autoGeneratedLines].join('\n')
